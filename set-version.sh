...
#   4. git add those files, commit, and push to  origin/gr2fork-main
#      (gated: must be a git repo on gr2fork-main with an origin remote;
#       a final y/N confirm precedes the push)
#
# Usage:
#     ./set-version.sh [ROOT]
#         ROOT  the combined repo dir (default: /home/jlee/Downloads/safe/combined)
#
...
# --------------------------------------------------------------- configuration ---
ROOT="${1:-/home/jlee/Downloads/safe/combined}"
BRANCH="gr2fork-main"
REMOTE="origin"
...
if [[ "$CUR_BRANCH" != "$BRANCH" ]]; then
    echo "ERROR: repo is on branch '$CUR_BRANCH', not '$BRANCH'." >&2
    echo "       Switch first:  git -C \"$REPO_ROOT\" checkout $BRANCH" >&2
    exit 1
fi
...
echo "Repo    : $REPO_ROOT  (branch $CUR_BRANCH, remote $REMOTE)"
echo "Current state:"
