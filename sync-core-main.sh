...
SRC_DIR="${SRC_DIR:-src}"
ROOT="${ROOT:-/home/jlee/Downloads/safe/gr2fork-new}"   # local combined checkout (ROOT=. to use cwd)
...
Config via env: ROOT (local checkout, default gr2fork-new), UPSTREAM_REMOTE (upstream), UPSTREAM_URL, UPSTREAM_BRANCH (main), SRC_DIR (src)
EOF
}
...
# --- enter the local checkout, then pin to its repo root ---
cd "$ROOT" 2>/dev/null || die "could not enter ROOT: $ROOT  (set ROOT=/path, or ROOT=. for the current dir)."
git rev-parse --is-inside-work-tree >/dev/null 2>&1 || die "not inside a git work tree (ROOT=$ROOT)."
