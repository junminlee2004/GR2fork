...
- **`core_gr2`** - the editable gr2fork (all features + the accuracy work ported in).
...
`x86-64-v3` floor if `CMAKE_C/CXX_FLAGS` carry no `-march`), so a znver4 build isn't clobbered.
The usual invocation works as-is; add `-DCOMBINED_EMBED_CORES=ON` for the single file.

**znver4 (the primary build):**
...
## What's proven vs. what needs a full toolchain

**Proven here**, by compile-and-run and by a real `cmake`+`ninja` build/configure:

- *Mechanism* (`verify_mechanism.sh`, 11/11 under both gcc and clang/ThinLTO/lld):
...
- *Umbrella, stub-source, the target flags* (`clang++ -O3 -flto=thin -fuse-ld=lld`,
...
**Needs a full toolchain** (without Vulkan/Boost/SDL3 and with the `externals/`
submodules unfetched, the real TUs aren't compiled): the actual per-core
compile + link of ~346/362 TUs against the vendored `externals/`. `find_package` floors
mirror the trees' CMakeLists; externals supply most via `OVERRIDE_FIND_PACKAGE`.

Run `verify_mechanism.sh` first - it re-proves the isolation contract locally in seconds:
