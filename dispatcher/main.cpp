...
//
// The one-and-only exe on disk. It selects a core and hands the process to it.
// Selection precedence:
//   1. --core=gr2|main      (consumed here, NOT forwarded to the core)
//   2. $SHADPS4_CORE         (gr2|main)
//   3. AUTO by the game's TITLE_ID: a Gravity Rush 2 / Gravity Rush Remastered
//      title -> core_gr2 (the fork); anything else -> core_main (prerelease).
//   4. default (no game / no title id resolvable): core_main.
//
// The title id is read the same way the emulator resolves it -- from the game's
// sce_sys/param.sfo (PSF, key "TITLE_ID") -- but BEFORE a core is loaded, since the
// dispatcher must pick which emulator core (.so) to bring in. A game launched by
// bare id (e.g. -g CUSA04943) is also matched directly.
//
// Everything after selector removal is forwarded verbatim, with argv[0] preserved
// so the core sees a sensible program name (the emulator stores it as
...
// Resolve the game's param.sfo from argv. A candidate game path is either a
// directory (<dir>/sce_sys/param.sfo) or a file like eboot.bin
// (<dir>/sce_sys/param.sfo, i.e. the file's parent). The explicit -g/--game value
// is preferred; otherwise every argument is tried and the first that yields an
// existing param.sfo wins (robust to option ordering).
