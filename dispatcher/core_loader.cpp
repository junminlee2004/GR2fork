...
//
// Cross-platform core loader. Loads exactly one core (a shared object/DLL that
// exports a single `core_entry` symbol) and runs it.
//
//   - Linux/ELF : dlopen/dlsym; single-file = memfd_create + dlopen("/proc/self/fd/N").
//   - Windows/COFF: LoadLibraryExW/GetProcAddress; single-file = extract the
//                   embedded bytes to a uniquely-named %TEMP% file + LoadLibrary
//                   (Windows has no memfd/fexecve equivalent).
//
// The two cores never interpose: on ELF the version script localizes everything
// but core_entry and each is dlopen'd RTLD_LOCAL; on COFF __declspec(dllexport)
// makes core_entry the sole export and each DLL resolves its own imports at link
// time, so there is no shared global symbol namespace to collide in.
...
#    include <unistd.h>
#  endif
// Provided by the .incbin stub (cmake/embed_cores*.S.in). Each core's shared
// object/DLL is embedded as a byte blob bracketed by these symbols. x86_64 C
// symbols have no leading underscore on either ELF or COFF, so these names match
// the assembler labels verbatim.
