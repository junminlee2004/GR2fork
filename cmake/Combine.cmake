...
# Host-shader sources: read PER TREE from the tree's own
# src/video_core/host_shaders/CMakeLists.txt (its set(SHADER_FILES ...) list), so
# an upstream sync that adds/removes a shader is picked up automatically and the
# two trees may diverge. A reconfigure is triggered when that file changes.
function(combine_read_shader_files src out_var)
    set(cml ${src}/video_core/host_shaders/CMakeLists.txt)
    if(NOT EXISTS ${cml})
        message(FATAL_ERROR "no host_shaders CMakeLists at ${cml} — the tree layout changed; "
                            "update combine_read_shader_files() in cmake/Combine.cmake.")
    endif()
    file(READ ${cml} _txt)
    if(NOT _txt MATCHES "set\\(SHADER_FILES([^)]*)\\)")
        message(FATAL_ERROR "cannot find set(SHADER_FILES ...) in ${cml} — upstream changed "
                            "the shader-list mechanism; update cmake/Combine.cmake.")
    endif()
    string(REGEX REPLACE "[ \t\r\n]+" ";" _list "${CMAKE_MATCH_1}")
    list(REMOVE_ITEM _list "")
    set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS ${cml})
...
function(combine_host_shaders name src out_var)
    ...
    combine_read_shader_files(${src} tree_shader_files)
    set(headers "")
    foreach(filename IN LISTS tree_shader_files)
...
function(combine_resources name src)
    ...
    # (0) EXPLICIT list (resdir-relative), passed by add_core(... CMRC_FILES ...).
    #     core_main gets upstream's exact embedded-resources list this way (extracted
    #     into CoreMainSurface.cmake by upstream-merge.py); trees without a list fall
    #     back to the derivation below.
    if(ARGC GREATER 2)
        foreach(rel IN LISTS ARGN)
            if(EXISTS ${resdir}/${rel})
                list(APPEND cmrc_rel ${rel})
            else()
                message(FATAL_ERROR "core_${name}: embedded resource missing on disk: "
                                    "${resdir}/${rel} (stale CMRC_FILES list?)")
            endif()
        endforeach()
    endif()
...
#   EXTRA_SOURCES compiled sources the *.cpp/*.s/*.S glob misses (e.g. .c), rel. to SRC
#   CMRC_FILES    explicit embedded-resource list (relative to the tree's resources/
#                 or images/ dir); empty -> combine_resources derives the set
# ===========================================================================
function(add_core)
    cmake_parse_arguments(AC "" "NAME;SRC" "EXCLUDES;LIBS;DEFINES;EXTRA_SOURCES;CMRC_FILES" ${ARGN})
    set(name ${AC_NAME})
    set(src  ${AC_SRC})
    ...
    endforeach()
    foreach(extra IN LISTS AC_EXTRA_SOURCES)
        list(APPEND core_srcs ${src}/${extra})
    endforeach()

    # ---- shadNet protobuf codegen (upstream 0.16.x): a tree that ships
    #      src/shadnet/shadnet.proto gets a per-core protoc run mirroring upstream's
    #      root-CMakeLists custom command. The generated .pb.cc is compiled into the
    #      core; libprotobuf itself arrives via the core's LIBS (CORE_MAIN_LIBS).
    set(pb_out "")
    if(EXISTS ${src}/shadnet/shadnet.proto)
        if(NOT TARGET protobuf::protoc)
            message(FATAL_ERROR
                "core_${name}: ${src}/shadnet/shadnet.proto needs protobuf::protoc from "
                "externals/protobuf, which isn't built. Sync externals (./upstream-merge.py) "
                "or: git submodule update --init --recursive externals/protobuf")
        endif()
        set(pb_out ${gen}/shadnet_proto_gen)
        file(MAKE_DIRECTORY ${pb_out})
        add_custom_command(
            OUTPUT  ${pb_out}/shadnet.pb.cc ${pb_out}/shadnet.pb.h
            COMMAND protobuf::protoc
                    "--proto_path=${src}/shadnet" "--cpp_out=${pb_out}"
                    "${src}/shadnet/shadnet.proto"
            DEPENDS "${src}/shadnet/shadnet.proto"
            COMMENT "core_${name}: generating shadnet protobuf sources"
            VERBATIM)
        list(APPEND core_srcs ${pb_out}/shadnet.pb.cc)
    endif()
    ...
    combine_resources(${name} ${src} ${AC_CMRC_FILES})
    ...
    if(pb_out)
        # mirrors upstream's target_include_directories(shadps4 PRIVATE ${SHADNET_PROTO_OUT})
        target_include_directories(core_${name} PRIVATE ${pb_out})
    endif()
