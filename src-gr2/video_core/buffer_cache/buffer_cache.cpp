...
struct VbbResolveCache {
    ...
    // GR2FORK v4.x: cached host-side bind result + per-ObtainBuffer-call sync
    // list, so a pure cmdbuf rotation (pipeline + stamp + buffer-generation all
    // unchanged) can re-emit setVertexInputEXT + bindVertexBuffers straight
    // from the cache, skipping the GetVertexInputs walk, both hash loops,
    // FindBuffer, and the sort/merge/map pass. host_generation +
    // IsBufferInvalid recheck guard the cached buffer_ids/handles; content sync
    // (SynchronizeBuffer) still runs every rotation because CPU writes are not
    // a registration event. host_buffers.size() == num_buffers.
...
void BufferCache::ReadMemory(VAddr device_addr, u64 size, bool is_write) {
    ...
    // FIX(GR2FORK): defense-in-depth bound check. The real gate is
    // Rasterizer::IsMapped (now overflow-safe), so a wild device_addr should
    // never reach here — but guard anyway, because page_table[page] ->
    // operator[] splits into first_level_map[page >> SecondLevelBits], and an
    // out-of-range page reads a wild first_level_map slot and AVs on Windows.
    // The dead read below is KEPT for valid pages (its fault-thread L1-page
    // allocation is load-bearing against the Jirga infinite-load freeze);
    // only impossible pages are skipped here, so unlike v3.1 this cannot
...
void BufferCache::BindVertexBuffersFixed(const Vulkan::GraphicsPipeline& pipeline,
    ...
    // (B') Cmdbuf re-emit fast path. OPT(GR2FORK v4.x): the slowdown the GRR
    // (accurate) path pays over Legacy is the per-cmdbuf re-emit forced by the
    // vbb_tick guard — at every cmdbuf rotation, an unchanged bind state still
    // re-runs FindBuffer + the sort/merge/map pass to rebuild host handles.
    // But those host handles/offsets are a pure function of the guest buffers
    // (fixed when pipeline+stamp are unchanged -> reuse_resolve) and the buffer
    // table (fixed when BufferRegistryGeneration() is unchanged): no buffer was
    // freed/relocated, so the cached vk::Buffer handles + offsets are still
    // exact. So reuse them and re-emit only the two cmdbuf-local driver calls.
    // Content sync still runs (CPU writes aren't a registration event); on the
    // common clean buffer it is the v1.56 lockless dirty-bit peek.
    if (reuse_resolve && vbb_resolve_->host_valid &&
        vbb_resolve_->host_generation == BufferRegistryGeneration()) [[likely]] {
        // Defence in depth: the generation gate already implies every cached
        // buffer_id is live (DeleteBuffer -> Unregister bumps the generation),
        // so this recheck is expected to never bail. If it ever does, fall
        // through to the full host-map path, which re-resolves and re-caches.
        bool all_live = true;
    ...
    // GR2FORK v4.x: capture per-ObtainBuffer-call slot buffer ids for the (B')
    // cmdbuf-rotation reuse path. host_cacheable goes false if any range
    // resolves to the shared stream buffer — its offset is cmdbuf/tick-local
    // and must be recomputed every rotation — so the cache is armed only when
    // every binding is a stable slot buffer.
    ...
    // GR2FORK v4.x: arm the (B') cmdbuf-rotation reuse cache. Only when every
    // binding resolved to a stable slot buffer. host_generation is captured
    // AFTER all ObtainBuffer calls so it reflects any buffer those calls
    // created; the next rotation's (B') compare then sees the table as we leave
    // it. Content sync is intentionally NOT cached — (B') re-runs
    // SynchronizeBuffer per range every rotation.
    if (host_cacheable) {
...
std::pair<Buffer*, u32> BufferCache::ObtainBuffer(VAddr device_addr, u32 size, bool is_written,
    ...
    if (!is_written && size <= CACHING_PAGESIZE && !IsRegionGpuModified(device_addr, size)) [[likely]] {
        ...
        // 2 × 32 = 64 bytes, exactly one cache line under alignas(64).
        // Cache footprint: 4096 × 64 = 256 KB per thread. Comfortably within
        // Z1 Extreme's 1 MB L2 per core. (The v3.6 4-way associativity bump
        // was reverted; the 32-byte four-tuple entry is from the epoch removal.)
        ...
        // Probe both ways. way 0 is hit in the dominant case; way 1 only
        // matters when way 0 was evicted by a colliding key. The
        // `addr == device_addr` check folds in the invalid-sentinel test
        ...
        // cache.lru[set_idx] names the next victim; after the insert it
        // flips to the other way so that way evicts next.
        const u64 offset = stream_buffer.Copy(device_addr, size, instance.UniformMinAlignment());
...
void BufferCache::ChangeRegister(BufferId buffer_id) {
    ...
    // PERF(GR2FORK v4.x): any (un)registration invalidates cached BufferId /
