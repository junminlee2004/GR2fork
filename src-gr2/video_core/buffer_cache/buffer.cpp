...
Buffer::Buffer(const Vulkan::Instance& instance_, Vulkan::Scheduler& scheduler_, MemoryUsage usage_,
    ...
    // Compute dispatches (detile, post-process, async compute) touch the same guest buffers as
    // graphics. With a dedicated compute family the buffer is declared eConcurrent across
    // [graphics, compute] so cross-queue access skips ownership-transfer barriers.
...
void Buffer::Fill(u64 offset, u32 num_bytes, u32 value) {
    // GR2FORK FIX: refuse a transfer to a null or out-of-range destination.
    // A moved-from / failed-alloc buffer has handle VK_NULL_HANDLE, and a DMA
    // to a guest address that resolves to the 16-byte null buffer overruns it;
    // either issues a fixed-function write at ~0x0 or past the allocation ->
    // GPU WRITE_INVALID -> DEVICE_LOST (device lost, no descriptor-sink line).
    // Skipping is safe (the write was going to fault); the log names it.
    if (Handle() == VK_NULL_HANDLE || offset + num_bytes > size_bytes) [[unlikely]] {
        LOG_CRITICAL(Render_Vulkan,
                     "GR2 transfer-sink: skipped FillBuffer handle_ok={} offset={:#x} "
                     "bytes={:#x} size={:#x}",
                     Handle() != VK_NULL_HANDLE, offset, num_bytes, size_bytes);
        return;
    }
...
void StreamBuffer::Commit() {
    // GR2FORK PERF: on AMD/RADV and integrated GPUs the staging memory is already coherent;
    // non-coherent discrete-GPU staging is the outlier, so the flush/invalidate is unlikely.
    ...
    offset += mapped_size;
    // GR2FORK PERF: hoist the CurrentTick acquire-load; both the watch-extend and watch-append
    // paths need it, so a single load saves one atomic op.
    const u64 cur_tick = scheduler->CurrentTick();
    // GR2FORK PERF: after the first Commit per submission, the
    ...
    if (current_watch_cursor != 0) [[likely]] {
        ...
        // GR2FORK PERF: consecutive Commits within the same submission tick are the steady
        // state; the entry extends, and new-tick fall-through happens on submit boundaries.
    ...
    // GR2FORK PERF: WATCHES_INITIAL_RESERVE is 0x4000 (16384)
