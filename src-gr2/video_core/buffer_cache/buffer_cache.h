...
private:
    ...
    // PERF(GR2FORK v4.x): bumped by ChangeRegister (Register/Unregister). See
    // BufferRegistryGeneration() above. Gates the BindVertexBuffersFixed
    // cmdbuf-rotation host-bind reuse path: cached host VkBuffer handles /
    // offsets are only reused while this counter is unchanged.
