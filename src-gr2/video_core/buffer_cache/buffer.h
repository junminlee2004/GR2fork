...
struct UniqueBuffer {
    ...
    // GR2FORK FIX: transfer device and bda_addr on move. Dropping bda_addr
    // left a moved Buffer with bda_addr=0 (the assert in BufferDeviceAddress
    // is compiled out under NDEBUG), so the BDA page table got near-null
    // entries and shader writes through them faulted at ~0x0. A Buffer moves
    // when slot_buffers grows on new-resource creation.
...
public:
    ...
    std::optional<vk::BufferMemoryBarrier2> GetBarrier(vk::AccessFlags2 dst_acess_mask,
                                                       ...
                                                       u32 offset = 0) {
        // GR2FORK PERF: consecutive draws using the same buffer with the same access pattern
        // (e.g. a UBO read across many draws in one render pass) hit this no-barrier early
        // return; only first-touch and access-mode transitions construct a barrier.
