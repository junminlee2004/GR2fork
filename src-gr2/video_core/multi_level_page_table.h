...
private:
    // GR2FORK FIX: the fault-handler and PM4/assembler threads both reach this; unsynchronized
    // first-touch ObjectPool::Create() corrupts the pool. Allocation stays on the fault thread
    // (moving it off refreezes loads); alloc_mutex guards the install, never held across a fault.
