...
TileManager::ScratchBuffer TileManager::GetScratchBuffer(u32 size) {
    ...
    // Scratch buffers are used by detile compute shaders and
...
void TileManager::TileImage(Image& in_image, std::span<vk::BufferImageCopy> buffer_copies,
    ...
    // End rendering before capturing cmdbuf rather than relying on Image::Download() doing it
    // internally after the capture. Same pattern as TileTextureFromBuffer.
    scheduler.EndRendering();
