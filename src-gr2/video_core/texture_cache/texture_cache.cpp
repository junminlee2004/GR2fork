...
TextureCache::TextureCache(const Vulkan::Instance& instance_, Vulkan::Scheduler& scheduler_,
    ...
    // GR2FORK M3 + T-MIRROR (2026-06-10): kill switches + boot echoes.
    // (Placed before the CanReportMemoryUsage early-return below so the
    // echoes fire on every device.) FORCED ON — no longer config-controlled;
    // disable only via the GR2_NO* env kills. The ctor runs post-config-load.
    {
...
void TextureCache::RegisterImage(ImageId image_id) {
    ...
    // GR2FORK T-MIRROR: Insert just set item.tick = gc_tick, so recording
    // gc_tick here establishes the invariant "mirror == gc_tick implies
    // item.tick >= gc_tick" from registration onward (re-registration of a
    // recycled slot re-runs this line, so stale mirrors can't survive).
...
void TextureCache::TouchImage(Image& image) {
    // GR2FORK T-MIRROR (2026-06-10): capture 2026-06-10-h put 4.06% of
    // BindResources SELF (~0.18%sys) on lru_cache.h:37 — the Touch
    // early-return compare reached through the item_pool deque's double
    // indirection (two LTO-inlined sites: the first-pass and second-pass
    // GetImage calls in BindTextures). gc_tick advances once per OnSubmit,
    // so in steady state nearly every Touch after an image's first this
    // tick no-ops AFTER paying the cold chase. Mirror the last touched
    // tick on the Image itself (a line the bind loop already has warm) and
    // skip the chase when it matches. lru_cache.Touch already no-ops on
    // item.tick >= tick, so the skip is bit-identical LRU evolution, not a
    // heuristic: mirror == gc_tick implies a Touch with this exact tick
    // already completed (invariant established at RegisterImage).
    // GR2_NOTOUCHMIRROR=1 restores the unconditional call.
...
void TextureCache::DeleteImage(ImageId image_id) {
    ...
    // GR2FORK M3: a Bloom filter can't clear per-key bits (other keys may
