...
Image::Image(const Vulkan::Instance& instance_, Vulkan::Scheduler& scheduler_,
    ...
    // FIX(GR2FORK v4.6 hardening, squidbus review): eCubeCompatible used to be
    // granted only when the FIRST binding of this memory was a cube T#
    // (props.is_cube). But the game may bind the same memory as a non-cube
    // first — a 2D-array T#, or a color target while rendering the faces — and
    // a later eCube view of that image then violates
    // VUID-VkImageViewCreateInfo-image-01003 (squidbus repro: Bloodborne). The
    // T# seen at creation is not a reliable signal, so instead request the flag
    // for ANY image that could legally back a cube view, per the
    // VkImageCreateInfo valid usage: 2D, square, >= 6 layers, single-sample.
    // This is the same shape predicate AmdGpu::Image::GetViewType applies on
    // the T# before requesting an eCube view, so the view request and the image
    // capability cannot disagree. Toggle-gated so gr2FixNativeCubeViews=false /
    // GR2_NOCUBEVIEW=1 keeps the exact v4.4 creation flags (no cube views are
    // requested with the toggle off, so the flag is never needed there).
    ...
    if (image_format_properties.result == vk::Result::eErrorFormatNotSupported &&
        (flags & vk::ImageCreateFlagBits::eCubeCompatible)) {
        // The cube_capable heuristic above widens eCubeCompatible beyond actual
        // cube T#s. If this driver rejects the format/flag combination, drop the
        // flag and retry rather than creating an image the driver refused. A cube
        // view of this image then becomes impossible; ImageView downgrades such a
        // request to e2DArray with a LOG_ERROR naming the image.
    ...
    // Record the FINAL cube capability (post drop-and-retry) so ImageView can
    // verify an eCube request against what this VkImage was actually created
    // with. props bits are outside every cache key/hash (UpdateSize packs only
    // is_block/is_pow2), so setting this here is hash-benign.
...
void Image::SetBackingSamples(u32 num_samples, bool copy_backing) {
    ...
    if (it == backing_images.end()) {
        ...
        if (new_image_ci.samples != vk::SampleCountFlagBits::e1) {
            // VUID-VkImageCreateInfo-samples-02257: a multisampled image cannot be
            // cube compatible. Cube T#s are single-sampled, so an MSAA backing is
            // never the target of a cube view; drop the flag the single-sample
