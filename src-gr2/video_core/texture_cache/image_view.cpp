...
ImageViewInfo::ImageViewInfo(const AmdGpu::Image& image, const Shader::ImageResource& desc) noexcept
    ...
    range.extent.levels = image.NumViewLevels(desc.is_array, desc.is_raw_access);
    range.extent.layers = image.NumViewLayers(desc.is_array, desc.is_raw_access);
    type = image.GetViewType(desc.is_array, desc.is_raw_access);
...
ImageView::ImageView(const Vulkan::Instance& instance, const ImageViewInfo& info_,
    ...
    // Last-resort guard (squidbus review, Bloodborne VUID-VkImageViewCreateInfo-
    // image-01003): an eCube view of an image created without eCubeCompatible is
    // invalid usage. With the creation-side cube_capable heuristic and the
    // GetViewType shape gate this should be unreachable; if a path slips through
    // (e.g. the driver rejected the flag at creation), downgrade to e2DArray.
    // Sampling will be wrong for this binding — the shader expects Dim=Cube —
    // but the API stays valid and the log names the exact image.
    vk::ImageViewType view_type = ConvertImageViewType(info.type);
    if ((view_type == vk::ImageViewType::eCube || view_type == vk::ImageViewType::eCubeArray) &&
        !image.info.props.is_cube_capable) {
        LOG_ERROR(Render_Vulkan,
                  "Cube view requested on non-cube-capable image {}x{}x{} layers={} format={} "
                  "at {:#x}; downgrading to 2D array",
                  image.info.size.width, image.info.size.height, image.info.size.depth,
                  image.info.resources.layers, vk::to_string(image.info.pixel_format),
