...
struct Image {
    ...
    Image(Image&& other) noexcept
        : instance(other.instance), scheduler(other.scheduler),
          ...
          view_format_list_(other.view_format_list_),
          view_format_count_(other.view_format_count_),
          ...
          lru_touch_tick(other.lru_touch_tick),
    ...
    // GR2FORK FIX: a transfer/clear to a null image handle (no backing, or a
    // failed vmaCreateImage) issues a fixed-function write at ~0x0 ->
    // WRITE_INVALID -> DEVICE_LOST with no descriptor-sink line. Guards below
    // gate every transfer method on this.
    bool HasBacking() const noexcept {
        return backing != nullptr && backing->image.image != VK_NULL_HANDLE;
    }
    ...
    /// Fast inline check if image is already in the target state.
    ...
    // GR2FORK PERF: process-wide image layout/backing generation, bumped by the three audited
    // mutation points (GetBarriers' two state commits, SetBackingSamples). BeginRendering's cache
    // stamps it to prove "nothing changed since populate" with one compare. Relaxed atomic.
    [[nodiscard]] static u64 LayoutGeneration() noexcept {
        return layout_generation_.load(std::memory_order_relaxed);
    }
    static inline std::atomic<u64> layout_generation_{1};
...
public:
    ...
    // GR2FORK PERF: the view-format list this VkImage was created with (count 0 = listless, any
    // compatible format). Consulted by FindView's violation probe and by SetBackingSamples when
    // re-chaining the copied create-info.
    std::array<vk::Format, 2> view_format_list_{};
    u32 view_format_count_{0};
    struct State {
    ...
    struct BackingImage {
        ...
        // Hot-path ImageView lookup cache (last-hit).
    ...
    u64 lru_id{};
    // GR2FORK: gc_tick of the last LRU touch, mirrored onto the Image so TextureCache::TouchImage
    // can early-out on this warm field instead of chasing lru_id through the LRU deque's double
    // indirection. Maintained by RegisterImage + TouchImage only.
    u64 lru_touch_tick{};
    u64 tick_accessed_last{};
    u64 hash{};

    // Atomic fast-state for the lock-free UpdateImage() fast path: most calls (~80%+) find the
    // image clean, tracked, and recently touched, yet the shared_lock costs 4.27% of L1D misses.
    // {dirty_bit, tracked_bit, last_touch_tick} pack into one u64 read with a single atomic load.
