...
public:
    ...
    bool IsMeta(VAddr address) const {
        // GR2FORK M3 (2026-06-10): one-cache-line Bloom prefilter over the
        // surface_metas key set. BindTextures consults IsMeta once per image
        // binding per draw (defensive meta-read detection) and the dispatch
        // skip heuristic consults it per CS buffer descriptor. surface_metas
        // is keyed by raw, 256-byte-aligned guest addresses through
        // std::hash<u64> (identity hash), so robin-hood probing in the
        // power-of-two table degenerates into long cluster runs — capture
        // 2026-06-10-h put 7.04% of BindResources SELF (~0.31%sys) on the
        // inlined probe loop (robin_hash.h:1114/:1116). The filter answers
        // the ~always "no" from one hot cache line; positives (real or
        // false) fall through to the real map, so the returned answer is
        // bit-identical to the unfiltered consult. GR2_NOMETABLOOM=1
        // restores the direct map lookup.
...
private:
    ...
    // GR2FORK M3 (2026-06-10): 512-bit (one cache line) Bloom filter over
    // surface_metas keys; see IsMeta. Maintained at the three emplace sites
    // (FindRenderTarget cmask/fmask, FindDepthTarget htile) and rebuilt from
    // the map after DeleteImage erases (erase can't clear shared bits). All
    // mutation and the hot consult run on the GpuAssembler resolve thread —
    // same access class as surface_metas itself, no atomics needed.
    ...
    // GR2FORK B3 ride-along (2026-06-10): capture i showed the array landing
    // at object+0x3588 (&0x3F = 0x8), straddling two lines; pin it to one.
    alignas(64) std::array<u64, 8> meta_bloom_{};
    bool meta_bloom_enabled_{true};
    // GR2FORK T-MIRROR (2026-06-10): see TouchImage in the .cpp.
