...
ImageInfo::ImageInfo(const AmdGpu::Image& image, const Shader::ImageResource& desc) noexcept {
    ...
    // GR2FORK v4.6 hardening (squidbus): props.is_cube records that THIS info
    // came from a cube T#. It no longer drives the eCubeCompatible create flag —
    // the game may bind the memory as a non-cube first, so Image::Image grants
    // the flag from a shape-based capability heuristic instead (2D + square +
    // >=6 layers + single-sample) and records the outcome in
    // props.is_cube_capable. Still gated on the skybox-fix toggle so toggle-off
    // restores the v4.4 plain-2D-array behavior verbatim.
