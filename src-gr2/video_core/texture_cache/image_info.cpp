...
ImageInfo::ImageInfo(const AmdGpu::Image& image, const Shader::ImageResource& desc) noexcept {
    ...
    // GR2FORK: under the skybox-fix toggle, props.is_cube only records that this info came from
    // a cube T#; Image::Image grants eCubeCompatible from a shape heuristic (2D + square + >=6
    // layers, single-sample; recorded in props.is_cube_capable) - memory may bind non-cube first.
    props.is_cube = image.IsCube() && AmdGpu::Gr2NativeCubeViewsEnabled();
    ...
    // GR2FORK FIX: layer counts > 2048 exceed Vulkan's minimum-spec maxImageArrayLayers and can
    // never be created; collapse such corrupt T#s (observed: a 4609-layer gallery T# OOM'ing a
    // 4 GiB device) to 1 layer. (1024, 2048] passes untouched; the AMD field is 13 bits (8191 max).
...
void ImageInfo::UpdateSize() {
    // No layer-count fixup for garbage T#s here - a cap in UpdateSize would also clobber the
    // ColorBuffer/DepthBuffer/null paths. The authoritative cap is the T# constructor above; the
    // orthogonal block/macro-tile garbage bail is in vk_rasterizer.cpp BindTextures.

    // GR2FORK PERF: signature over packed 64-bit words mixed by 4 independent FNV mixers (distinct
    // seeds, XOR-combined): a serial FNV-1a chain has a ~16-cycle critical path on Zen4 vs ~5
    // pipelined; UpdateSize is hot (~0.53% of GpuComm). size_sig collisions re-run mip layout only.
        ...
        switch (array_mode) {
            ...
            case AmdGpu::ArrayMode::Array2DTiledThin1: {
                if (props.is_block) [[unlikely]] {
                    // GR2FORK FIX: garbage T# - macro-tiled 2D mode is mutually exclusive with
                    // block-compressed formats; zero-size so BindTextures emits a null descriptor.
                    // Not an ASSERT: shipping titles (GR2 texture bombs) produce this combination.
                    LOG_WARNING(Render_Vulkan,
    ...
    for (s32 mip = 0; mip < resources.levels; ++mip) {
        ...
        case AmdGpu::ArrayMode::Array2DTiledThin1: {
            if (props.is_block) [[unlikely]] {
                // GR2FORK FIX: garbage T# - same zero-size-and-bail as the single-mip path
                // above. guest_size accumulated for earlier mips is overwritten to zero;
                // the caller drops the whole image anyway.
                LOG_WARNING(Render_Vulkan,
                            "T# garbage: Array2DTiledThin1 + block fmt — "
