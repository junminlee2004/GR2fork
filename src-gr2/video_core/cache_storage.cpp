...
void ProcessIO(const std::stop_token& stoken) {
    ...
    // GR2FORK: GetHotCoreFenceMask returns 0 in non-exclusive mode, so this
    // persistent compression-I/O worker is left unconstrained (no pin) and may
    // use the GpuComm/GpuAssembler cores when they are idle during a burst.
