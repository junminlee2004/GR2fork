...
// GR2FORK FIX: single-mutex producer-consumer queue for cache I/O - a separate request counter
// under its own mutex can drift from queue.size() and leave the worker spinning forever, so the
// queue itself is the sole source of truth. packaged_task runs its callable synchronously.
...
void ProcessIO(const std::stop_token& stoken) {
    ...
    // GR2FORK PERF: keep this persistent compression/disk worker off the hot cores - it does real
    // CPU work during compile bursts (MZ_BEST_COMPRESSION per blob) and inherits an all-CPU mask.
    // GetHotCoreFenceMask returns 0 in non-exclusive mode or when nothing is reserved: no pin then.
    if (const u64 forbidden = Common::GetHotCoreFenceMask()) {
...
// GR2FORK: crash-sentinel cache recovery. Corrupt blobs crash inside the load window (asserts /
// UNREACHABLE, not catchable as exceptions), so Open() arms a marker file before any blob is read
// and FinishPreload() disarms it; a launch finding the marker quarantines the dir as cache.old.
static constexpr char kLoadMarkerName[] = "cache_load.inprogress";

static void RotateCrashedCacheDir(const std::filesystem::path& cache_root) {
    std::error_code ec;
    const auto parent = cache_root.parent_path();
    const auto old_path = parent / "cache.old";
    const auto old_old_path = parent / "cache.old.old";
    if (std::filesystem::exists(old_path, ec)) {
        std::filesystem::remove_all(old_old_path, ec); // replace an older second backup
        std::filesystem::rename(cache_root, old_old_path, ec);
        LOG_WARNING(Render,
                    "Previous run crashed while loading the shader cache — quarantined {} as {}",
                    cache_root.string(), old_old_path.string());
    } else {
        std::filesystem::rename(cache_root, old_path, ec);
        LOG_WARNING(Render,
                    "Previous run crashed while loading the shader cache — quarantined {} as {}",
                    cache_root.string(), old_path.string());
    }
    if (ec) {
        LOG_ERROR(Render, "Cache quarantine rename failed: {}", ec.message());
    }
...
void DataBase::Open() {
    ...
    // Crash-sentinel check + arm (see RotateCrashedCacheDir doc above).
    {
        const auto cache_root = GetUserPath(PathType::CacheDir);
        std::error_code ec;
        if (std::filesystem::exists(cache_root / kLoadMarkerName, ec)) {
            RotateCrashedCacheDir(cache_root);
        }
        const IOFile marker{cache_root / kLoadMarkerName, FileAccessMode::Create};
    }
...
void DataBase::Close() {
    ...
    // GR2FORK FIX: drain pending writes BEFORE requesting stop - the worker exits on
    // stop_requested without draining, silently losing in-flight writes. The queue holds one
    // entry per blob and drains in milliseconds; cv.notify_one keeps the worker processing.
    while (true) {
        bool empty;
        {
...
void DataBase::FinishPreload() {
    ...
    // Disarm the crash sentinel: the risky load window is over. Failures
    // past this point are not cache-load crashes and must not trigger a
    // quarantine on the next launch.
    std::error_code ec;
    std::filesystem::remove(Common::FS::GetUserPath(Common::FS::PathType::CacheDir) /
                                kLoadMarkerName,
                            ec);
}
...
u32 DataBase::CountBlobs(BlobType type) {
    ...
    // Counts blobs of `type` from directory metadata (filesystem mode) or the zip index (archived
    // mode) without reading data - sizes the LOADING SHADERS progress bar. Even a 41,896-pipeline
    // cache scans in under 100 ms.
    const auto& ext = GetBlobFileExtension(type);
    u32 count = 0;
    if (Config::isPipelineCacheArchived()) {
        const auto num_files = mz_zip_reader_get_num_files(&zip_ar);
