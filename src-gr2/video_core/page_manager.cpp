...
struct PageManager::Impl {
    ...
    void UpdatePageWatchers(VAddr addr, u64 size) {
        ...
        // GR2FORK PERF: this IsMapped lookup (TLS cache miss means shared lock + interval-set
        // find) runs on every watcher update purely to gate a diagnostic. Off by default;
        // GR2_TRACKDIAG=1 enables it when debugging mapping issues.
        static const bool track_diag = []() noexcept {
            const char* e = std::getenv("GR2_TRACKDIAG");
            return e && e[0] == '1';
        }();
        if (track_diag && !rasterizer->IsMapped(aligned_addr, aligned_end - aligned_addr))
            [[unlikely]] {
            LOG_WARNING(Render,
