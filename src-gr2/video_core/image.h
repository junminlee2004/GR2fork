...
public:
    ...
    // Lock-free fast state for the UpdateImage() fast path, hit for every image binding on every
    // draw. ~80%+ of calls find the image clean, tracked and recently touched; a shared_lock for
    // that case costs 4.27% of L1D misses. One u64 packs {dirty, tracked, last_touch_tick}.
