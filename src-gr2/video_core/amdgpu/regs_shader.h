...
// Non-fatal binary-info search. Returns nullptr when no valid BinaryInfo block
// is found within the scan window, instead of aborting. PS4 shaders carry an
// "OrbShdr" footer; a stage whose program pointer is non-null but points at
// mapped-but-non-shader memory (a stale pointer reused across pipelines, or
// memory the guest streamed over before an async compile ran) has no such
// footer. Callers that can skip such a stage should use this / TryGetParams;
// the asserting SearchBinaryInfo wrapper below is kept for paths that require
// success.
static constexpr const BinaryInfo* TrySearchBinaryInfo(const u32* code) {
...
static constexpr const BinaryInfo& SearchBinaryInfo(const u32* code) {
    if (const BinaryInfo* info = TrySearchBinaryInfo(code)) {
        return *info;
    }
...
// Optional form of GetParams: std::nullopt when the shader has no locatable
// binary info, so the caller can skip the stage instead of crashing.
static constexpr std::optional<Shader::ShaderParams> TryGetParams(const auto& sh) {
    const auto* code = sh.template Address<u32*>();
    const BinaryInfo* bininfo = TrySearchBinaryInfo(code);
    if (bininfo == nullptr) {
        return std::nullopt;
    }
