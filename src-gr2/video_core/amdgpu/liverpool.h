...
private:
    ...
    // (GR2FORK L1-PROBE removed 2026-06-10 — investigation concluded. Final:
    // 2,097,152 transitions / 7,571,700 raises / 77 distinct offsets / 0 dropped.
    // Top transition raisers: CTX+0x205 PA_SU_SC_MODE_CNTL (polygon_control)
    // 55.6% @98.6% transition rate · SH+0x48 SPI_SHADER_PGM_LO_VS 30.9% ·
    // SH+0x8 PGM_LO_PS 3.9% · CTX+0x290 VGT_GS_MODE 3.7% · UCFG+0x242
    // VGT_PRIMITIVE_TYPE 1.4%; everything else ≤1.2%. Verdict: the D3'
    // pattern repeats, concentrated in ONE register → the M2-POLYGON
    // classification in liverpool.cpp's SetContextReg handler, which carries
    // the full derivation. Shader switches (#2/#3) are genuine key changes —
    // the honest residual. Per the probe rule (~0.5% each, removed at
    // conclusion) the MarkGfxKeyDirtyAt offset plumbing + 512-slot histogram
    // + dtor flush are deleted; all call sites are back on this function.)
    ...
    bool gfx_key_dirty_{};
    // GR2FORK: set when a key-affecting CONTEXT register changed (the subset of
    // gfx_key_dirty_ causes that is NOT a user_data-only SH re-emit). Lets
    // RefreshGraphicsKey skip rebuilding the context-derived key fields on the
    // GR2 per-draw SRT-pointer churn while still re-resolving modules. Set only
    // by MarkGfxKeyDirty; deliberately NOT set by MarkGfxKeyDirtyNoDynamic.
