...
private:
    // Active ring depth and wrap mask, chosen once at construction from the
    // session's title detection (see ActiveNumSlots). const so the SPSC
    // indexing math is fixed for the pool's lifetime. Declared before slots_
    // so the init list (which derives slot_mask_ from num_slots_) is in
    // declaration order.
