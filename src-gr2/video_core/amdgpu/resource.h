...
// GR2FORK: skybox seam fix - native cubemap views. On (default), a single-cube T# becomes a
// Vulkan eCube view for seamless cross-face filtering (FixCubeCoords rebuilds the coordinate);
// off falls back to legacy Color2DArray sampling. Kill switches: GR2_NOCUBEVIEW=1 or
// [GPU] gr2FixNativeCubeViews=false. Vulkan::Instance forces it off on Polaris (GCN4), whose
// drivers mishandle eCubeCompatible/eCube; Instance init precedes the first-use latch below.
inline std::atomic<bool> g_gr2_cube_views_hw_blocked{false};

inline void BlockGr2NativeCubeViewsForHw() noexcept {
    g_gr2_cube_views_hw_blocked.store(true, std::memory_order_relaxed);
}
...
inline bool Gr2NativeCubeViewsEnabled() noexcept {
    static const bool enabled = []() noexcept {
        ...
        return !(e && e[0] == '1') && Config::gr2FixNativeCubeViews() &&
               !g_gr2_cube_views_hw_blocked.load(std::memory_order_relaxed);
    }();
    return enabled;
}
...
struct Image {
    ...
    ImageType GetViewType(const bool is_array, const bool is_raw_access) const noexcept {
        const auto base_type = GetType();
        if (IsCube()) {
            // GR2FORK FIX: expose a single cubemap (depth==0, covering pow2-padded 6->8 layer
            // cubes) as a native eCube view - 2D-array sampling cannot filter across face edges
            // (hairline skybox seams). Cube arrays (depth>0) stay 2D-array (eCube needs exactly
            // 6 layers); !is_raw_access keeps IMAGE_LOAD/STORE/atomics on the legacy path; the
            // width == height && NumLayers() >= 6 test mirrors Image::Image's cube_capable
            // predicate, so shader spec, view request, and image capability cannot disagree.
...
struct Sampler {
    ...
    float MaxAniso() const {
        ...
        case AnisoRatio::Sixteen:
            return 16.0f;
        default:
            // GCN reserves max_aniso encodings 5-7, but GR2 produces them (partially-initialized
            // S#s in the FAM/Lua sprite path). Saturate to 16x; the call site clamps to
