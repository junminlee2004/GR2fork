...
// GR2FORK v4.5 skybox seam fix toggle: native cubemap image views. When ON
// (default), a single-cube T# is exposed as a Vulkan eCube view so the GPU
// performs seamless cross-face filtering; FixCubeCoords rebuilds the
// 3D-direction coordinate to match. When OFF, GetViewType returns the legacy
// base type (Color2DArray), every Cube switch case downstream becomes
// unreachable, and FixCubeCoords falls back to the legacy (x-1, y-1, face)
// build — the exact v4.4 behavior. This is the PRIME Polaris suspect: the
// eCubeCompatible/eCube view path is the only v4.5 change that alters which
// image and view types the driver sees. Env kill GR2_NOCUBEVIEW=1;
// launcher-facing [GPU] gr2FixNativeCubeViews (default true) — the sole
// surviving config-controlled GR2 toggle. Latched on first use,
// post-config-load (first caller is image-info/recompiler construction
// during rendering).
inline bool Gr2NativeCubeViewsEnabled() noexcept {
    static const bool enabled = []() noexcept {
        const char* e = std::getenv("GR2_NOCUBEVIEW");
        return !(e && e[0] == '1') && Config::gr2FixNativeCubeViews();
    }();
    return enabled;
}
...
struct Image {
    ...
    ImageType GetViewType(const bool is_array, const bool is_raw_access) const noexcept {
        const auto base_type = GetType();
        if (IsCube()) {
            // FIX(GR2FORK): expose a single cubemap as a native cube view so the GPU
            // performs seamless cross-face filtering. The legacy path returned
            // base_type (Color2DArray) and sampled the cube as a 2D array, which
            // cannot filter across face edges and leaves hairline seams at the face
            // joins (visible e.g. on the GR2 skybox).
            //
            // A single cube is depth==0 (one full cube = 6 faces). NumLayers() can
            // report 8 here when pow2pad rounds the 6 faces up to a power of two —
            // that is still one cube, and an eCube view over the first 6 layers is
            // valid (NumViewLayers returns 6). Gating on depth==0 therefore also
            // covers pow2-padded cubes that a NumLayers()==6 check would miss.
            //
            // Cube arrays (depth>0, i.e. >1 cube) stay 2D-array: a Vulkan eCube view
            // needs exactly 6 layers, and a cube array would also need a 4-component
            // (direction + cube index) coordinate that FixCubeCoordsNative does not
            // build.
            //
            // HARDENED (squidbus review, Bloodborne):
            //  - !is_raw_access: native cube views are for SAMPLED access only.
            //    IMAGE_LOAD/STORE/atomics address a cube as raw (x, y, face)
            //    2D-array texels and gain nothing from a cube view; OpImageFetch
            //    is invalid SPIR-V for Dim=Cube; and Metal (MoltenVK) cannot
            //    write cube textures at all. Any raw access on the binding keeps
            //    it on the legacy 2D-array path.
            //  - width == height && NumLayers() >= 6: the VkImage valid-usage
            //    conditions for eCubeCompatible, encoded from the T# alone so
            //    this decision can never request a cube view that the backing
            //    image cannot legally provide. Image::Image grants the create
            //    flag from the same shape test (see the cube_capable heuristic
            //    there), so the shader spec, the view request, and the image
            //    capability are decided by one predicate and cannot disagree.
            //    Malformed cube T#s fall back to the legacy path.
            //
            // Toggle off (gr2FixNativeCubeViews=false / GR2_NOCUBEVIEW=1) returns
            // base_type below — the legacy 2D-array sampling, exact v4.4.
            if (depth == 0 && !is_raw_access && width == height && NumLayers() >= 6 &&
                Gr2NativeCubeViewsEnabled()) {
                return ImageType::Cube;
            }
            // Cube array / raw access / ineligible shape: remain array type
            // regardless of instruction array specifier.
            return base_type;
        }
    ...
    [[nodiscard]] u32 NumViewLevels(const bool is_array, const bool is_raw_access) const noexcept {
        switch (GetViewType(is_array, is_raw_access)) {
        case ImageType::Color2DMsaa:
        case ImageType::Color2DMsaaArray:
            return 1;
        default:
    ...
    [[nodiscard]] u32 NumViewLayers(const bool is_array, const bool is_raw_access) const noexcept {
        switch (GetViewType(is_array, is_raw_access)) {
        case ImageType::Color1D:
        case ImageType::Color2D:
        case ImageType::Color2DMsaa:
        case ImageType::Color3D:
            return 1;
        case ImageType::Cube:
            // A cube view must cover exactly its 6 faces.
            return 6;
        default:
