...
// =========================================================================
// GR2FORK M2-POLYGON (2026-06-10): bit-aware key classification for
// PA_SU_SC_MODE_CNTL (polygon_control). Evidence + classifier in the
// SetContextReg handler below; this is the kill switch + the key-bit mask.
// =========================================================================
// Kill switch: GR2_NOPOLYCLASS=1 restores the unconditional MarkGfxKeyDirty
// classification verbatim (env read once per process; boot-echoed from the
// ctor). FORCED ON by default — no longer config-controlled (the v4.5 fps
// optimizations are always enabled; only the GR2_NO* env kills disable them).
// Latched at static init, before process_thread spawns, so Process() only
// ever observes the final value. A/B via the env kill, three-strikes.
static bool g_poly_class_enabled = []() noexcept {
    const char* e = std::getenv("GR2_NOPOLYCLASS");
    return !(e && e[0] == '1');
}();

// KEY-FEEDING BITS of PolygonControl (regs_primitive.h:69, audited 2026-06-10
// by exhaustive consumer grep — the M2-POLYGON comment in SetContextReg has
// the per-consumer breakdown):
//   bits 3-4   enable_polygon_mode     ┐ read by PolyMode() →
//   bits 5-7   polygon_mode_front      ┤   key.polygon_mode
//   bits 8-10  polygon_mode_back       ┘   (unread today; kept in the mask
//                                          conservatively — PolyMode() could
//                                          grow a back-face read)
//   bit  19    provoking_vtx_last      → key.provoking_vtx_last
// = 0x000807F8. Every other bit is dynamic-state-only (cull 0-1, front_face
// 2, polygon-offset enables 11-13, window_offset 16) or unread by any
// consumer (14-15, 17-18, persp_corr_dis 20, multi_prim_ib_ena 21).
static constexpr u32 kPolygonControlKeyMask = 0x000807F8u;

Liverpool::Liverpool() {
    num_counter_pairs = Libraries::Kernel::sceKernelIsNeoMode() ? 16 : 8;
    // M2-POLYGON is forced on (env kill GR2_NOPOLYCLASS=1 only); the value was
    // already latched from the env at static init, before process_thread can
    // observe it.
    LOG_INFO(Render,
    ...
    while (!dcb.empty()) {
        ...
        case 3:
            ...
            case PM4ItOpcode::SetContextReg: {
                ...
                // GR2FORK M2-POLYGON: capture the OLD polygon_control word
                // BEFORE the copy below overwrites it — the bit-aware
                // classifier at the bottom of this case needs old^new. Scoped
                // to exactly single-dword writes at polygon_control's offset
                // (the L1 probe measured 98.6% of its raises in that shape);
                // any other packet shape takes the conservative path. Cost on
                // the non-polygon path: one compare per packet, on GpuComm
                // (not the bottleneck thread).
                ...
                if (IsDynamicStateOnlyContextReg(reg_addr)) {
                    ...
                    // =====================================================
                    // GR2FORK M2-POLYGON (2026-06-10): bit-aware downgrade
                    // for PA_SU_SC_MODE_CNTL (polygon_control), the L1-probe
                    // verdict: 55.6% of ALL gfx_key_ctx_dirty transitions
                    // (1,165,644 of 2,097,152; 98.6% of its 1,182,758 raises
                    // were transitions = it is the first raiser before
                    // nearly every ctx-dirty resolve). GR2 toggles the
                    // cull/face bits per draw batch; the whole-dword write
                    // used to dirty the key and force the full
                    // RefreshGraphicsKey + BuildRuntimeInfo path.
                    //
                    // Consumer audit (exhaustive grep, D3' field-by-field
                    // standard): the KEY reads exactly PolyMode() (bits 3-7;
                    // 8-10 kept in the mask conservatively) at
                    // vk_pipeline_cache.cpp:1068 and provoking_vtx_last
                    // (bit 19) at :1066 — the kPolygonControlKeyMask bits.
                    // Everything else is DYNAMIC-side: CullingMode bits 0-1
                    // (vk_rasterizer.cpp:3055), front_face bit 2 (:3057),
                    // NeedsBias bits 11-13 (:3006), enable_window_offset
                    // bit 16 (:2889 scissor); bits 14-15/17-18/20-21 have no
                    // consumer anywhere. So when old^new misses the mask,
                    // this write is dynamic-state-only by the same proof
                    // standard as IsDynamicStateOnlyContextReg — just bit-
                    // granular on a mixed register.
                    //
                    // MarkGfxPipelineDirty keeps the stamp bump (next draw
                    // misses Level 1, passes Level 2 on gfx_key_dirty=false)
                    // and raises dynamic_dirty_ (cull/face/bias re-emitted by
                    // UpdatePrimitiveState/UpdateDepthStencilState). The
                    // binding-skip and br caches are stamp-keyed and
                    // revalidate conservatively. Only the key/ctx dirtiness
                    // is relaxed — D3' semantics, one register deeper.
                    // Kill switch GR2_NOPOLYCLASS (mask + switch defined
                    // above Liverpool's ctor).
                    // =====================================================
                    MarkGfxPipelineDirty();
                } else {
                    MarkGfxKeyDirty();
