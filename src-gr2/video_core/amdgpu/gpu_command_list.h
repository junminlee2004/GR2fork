...
// GPU command list: PM4 parsing emits lightweight command records, and a flush pass dispatches
// them to the rasterizer. The list is flushed before any data-changing pipeline-state register
// write and any sync point, so every queued command shares the same pipeline state in `regs`.
