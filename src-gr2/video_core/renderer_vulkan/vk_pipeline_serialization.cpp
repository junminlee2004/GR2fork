...
// v2: ImageResource gained is_raw_access (cube hardening) - InfoPersistent is
// raw-byte serialized, so its layout changed and v1 meta blobs must be rejected.
...
bool PipelineCache::LoadComputePipeline(Serialization::Archive& ar) {
    ...
    // GR2FORK: duplicate blobs (e.g. a zip re-finalized after a crash) must
    // not ASSERT-abort the boot; drop the duplicate instead - the
    // already-loaded pipeline stays authoritative.
    if (!is_new) {
        LOG_WARNING(Render, "Duplicate compute pipeline {:#018x} in shader cache — skipped",
                    compute_key.value);
        return false;
    }
...
bool PipelineCache::LoadGraphicsPipeline(Serialization::Archive& ar) {
    ...
    if (!is_new) { // GR2FORK: same duplicate-blob tolerance as the compute path
        LOG_WARNING(Render, "Duplicate graphics pipeline in shader cache — skipped");
        return false;
    }
...
bool PipelineCache::LoadPipelineStage(Serialization::Archive& ar, size_t stage) {
    ...
    // GR2FORK FIX: perm_idx-keyed dedup - same (pgm_hash, perm_idx) means the same SPV file and
    // an identical compiled module, so no spec comparison is needed. Spec-equality dedup can
    // false-match distinct specializations and cost ~1958 redundant CompileSPV calls per warmup.
...
void PipelineCache::WarmUp(const std::function<void(u32 loaded, u32 total)>& tick) {
    ...
    // GR2FORK FIX: a profile blob written by a build with a different
    // sizeof(profile) is incompatible; the size guard short-circuits before
    // the memcmp so a shorter blob is never read past its end.
    if (profile_data.size() != sizeof(profile) ||
    ...
    // Pre-count pipeline blobs so the LoadingScreenLayer overlay can render a real "X / Y"
    // progress bar. Metadata-only directory (or zip-index) walk - sub-100ms even for tens of
    ...
    Storage::DataBase::Instance().ForEachBlob(
        Storage::BlobType::PipelineKey, [&](std::vector<u8>&& data) {
            ...
            // GR2FORK: corrupt-blob tolerance. A garbage size-prefix throws length_error /
            // bad_alloc from a Reader container resize; a truncated read trips the serdes ASSERT
            // (handled by the cache_load.inprogress sentinel). Thrown failures skip the bad blob.
            bool result{};
            try {
                u32 version{};
                pldata.Read(version);
                ...
                static u32 logged = 0;
                if (logged++ < 8) {
                    LOG_ERROR(Render, "Corrupt shader-cache blob skipped ({}); it will be "
                                      "regenerated during play",
                              e.what());
                }
                result = false;
            }
            ...
            // Progress tick for the LoadingScreenLayer overlay, called on every blob (including
            // failed ones) so the bar animates smoothly past stale entries; the presenter's tick
            // handler throttles actual swapchain redraws to ~16fps.
            if (tick) {
