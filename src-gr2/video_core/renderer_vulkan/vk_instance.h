...
public:
    ...
    /// compute queues across a frame. Returns eConcurrent only
...
private:
    ...
    // Populated in CreateDevice with {graphics_family, compute_family} when async compute is
    // available; otherwise CrossQueueFamilyIndices returns an empty span. A fixed array gives a
    // stable pointer for VkBufferCreateInfo::pQueueFamilyIndices.
