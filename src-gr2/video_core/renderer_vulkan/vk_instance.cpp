...
Instance::Instance(Frontend::WindowSDL& window, s32 physical_device_index,
    ...
    // GR2FORK: Polaris (GCN4) runtime detection by PCI id (vendor 0x1002; Polaris10/11
    // 0x67C0-0x67FF, Polaris12 0x6980-0x699F, Polaris20 0x6FDF) with a RADV device-name "POLARIS"
    // fallback. On match, the native cube-view path is force-disabled before image-info is built.
    {
        const u32 vendor = properties.vendorID;
        const u32 device = properties.deviceID;
        const std::string_view name{properties.deviceName};
        const bool polaris_id = (device >= 0x67C0 && device <= 0x67FF) ||
                                (device >= 0x6980 && device <= 0x699F) || device == 0x6FDF;
        if (vendor == 0x1002 && (polaris_id || name.find("POLARIS") != std::string_view::npos)) {
            AmdGpu::BlockGr2NativeCubeViewsForHw();
            LOG_WARNING(Render_Vulkan,
                        "Polaris GPU detected ({}, id {:#x}) — native cube views force-disabled",
                        name, device);
        }
    }
...
bool Instance::CreateDevice() {
    ...
    // GR2FORK: prefer a compute-only queue family (a dedicated async-compute family; AMD/RADV
    // exposes one). If absent, fall back to the graphics family - async compute is then not
    // parallel, which callers detect via IsAsyncComputeAvailable() and route to graphics.
...
void Instance::CollectPhysicalMemoryInfo() {
    ...
    // Total physical device-local VRAM, computed up front so the allocation policy can be picked
    // before accumulating the reported budget below.
    u64 local_memory = 0;
    ...
    // VRAM-size-gated allocation strategy. Discrete cards under 9 GiB report the full physical
    // heap and skip the system reserve - heapBudget already deducts other apps' usage (~3367 of
    // 4096 MiB on a 3050 Ti). >= 9 GiB keeps the reserve; integrated GPUs force low_vram false.
