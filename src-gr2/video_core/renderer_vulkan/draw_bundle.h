...
// DrawBundle: self-contained package of pre-resolved Vulkan recording data - built by GpuComm,
// consumed by the Recorder thread issuing vkCmd* calls. Cache eviction drains the recorder queue
// via scheduler.Finish(), so handles stay valid; descriptor writes point into bundle storage.
...
struct DrawBundle {
    ...
    enum class Type : u8 {
        ...
        DrawIndirect,
        DrawIndexedIndirect,
        Dispatch,            // Compute direct dispatch
        DispatchIndirect,    // Compute indirect dispatch
    ...
    // Descriptor writes: GR2 pipelines with VS+FS+GS can produce 200+ on first bind. One buffer
    // info per buffer descriptor, one image info per texture/sampler/storage image descriptor.
    ...
    // Indirect args. Draw[Indexed]Indirect[Count] uses all fields (indirect_count_buffer is
    // non-null for the *Count variants); DispatchIndirect uses only indirect_buffer + offset.
    ...
    // Compute direct dispatch dimensions.
    u32 dispatch_dim_x{};
    u32 dispatch_dim_y{};
    u32 dispatch_dim_z{};
...
// DrawBundleRing: lock-free SPSC ring buffer for DrawBundles. The parser thread writes directly
// into pre-allocated slots via TryStartWrite/FinishWrite (zero-copy); the recorder thread reads
// them in order via TryStartRead/FinishRead.
