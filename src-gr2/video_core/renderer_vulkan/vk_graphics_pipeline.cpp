...
void GraphicsPipeline::GetVertexInputs(
    ...
    // GR2FORK PERF: vertex shaders without a fetch shader / input attributes exist (fullscreen
    // pass-through via gl_VertexIndex) but are a small minority of GR2's draws.
...
void GraphicsPipeline::BuildDescSetLayout(bool preloading) {
    ...
    // GR2FORK PERF: RADV push descriptors are a major CPU hot spot (memmove +
    // radv_cmd_update_descriptor_sets), so descriptor sets are the RADV default.
    // SHADPS4_FORCE_PUSH_DESCRIPTORS=1 / SHADPS4_DISABLE_PUSH_DESCRIPTORS=1 override.
