...
// GR2FORK: address-independent specialization fingerprint. Hashes the per-draw
// VARYING inputs to StageSpecialization::ComputeSig -- runtime_info, the
// binding start, and every bound sharp -- but reads each sharp with
// base_address ZEROED, so a pure pointer re-emit (GR2's per-draw UBO SGPR
// churn) hashes identically. base_address is provably NOT a ComputeSig input
// (the *Spec structs extract only format/type/dimension fields), so this is a
// SUPERSET of ComputeSig's per-draw inputs: it maps 1:1 to spec.sig and can
// only over-discriminate (a missed reclaim), never under-discriminate (a wrong
// reuse). Program-static inputs (pgm_hash, stage, fetch-shader layout, desc
// flags) are intentionally omitted -- the cache is keyed per-Program, so they
// are constant within it. Validated on GR2: 0 collisions over 12.2M samples.
// The caller excludes HS/DS (their spec also folds tessellation-constant-buffer
// CONTENTS read from guest memory, which this descriptor-only key cannot cover).
// PERF(GR2FORK STAGE-MEMO ri-fp): `ri_bytes_hash` is XXH3_64bits of the full
// RuntimeInfo struct bytes, supplied by the caller. It used to be computed
// here on every call; it is the largest single hash in this function (the
// ~200 B struct lands in XXH3's 129-240 length bucket — 0.44%sys in the
// post-memo profile, ~all from this site) and under a stage-memo hit it is a
// pure function of an untouched object, so GetProgram memoizes it per stage.
// The mix ORDER is unchanged, so produced fingerprints are byte-identical to
// the pre-change function and the 0-collision/12.2M validation carries over.
...
// GR2FORK STAGE-MEMO kill switch. Set GR2_NOSTAGEMEMO=1 to disable the
// Level-2.5 per-stage resolve memo CONSULT (populate is unaffected; the env
// is read once per process). FORCED ON — no longer config-controlled. First
// call happens during rendering, well after config load. A/B: run with and
// without at the usual spot and compare fps (the STAGE-MEMO probe counters
// were removed 2026-06-10).
...
// GR2FORK K1 kill switch. Set GR2_NOSTAGECMP=1 to restore the full-key
// memcmp at the Level-2.5 reuse check (the narrowed stage-region compare is
// the default; env read once per process; boot-echoed from the ctor). FORCED
// ON — no longer config-controlled. A/B: run with and without at the usual
// spot, three-strikes.
...
// GR2FORK STAGE-MEMO eligibility. Restricted to the (stage, l_stage) pairs
// whose BuildRuntimeInfo reads ONLY snapshot registers — all value-frozen
// under !gfx_key_ctx_dirty (see the StageResolveMemo doc block in
// vk_pipeline_cache.h). Vertex->Vertex reads vs_program.settings +
// vgt_instance_step_rate_* + vs_output_control + clipper_control +
// primitive_type; Fragment->Fragment reads ps_program.settings +
// ps_input_ena/addr + num_interp + z_export_format + depth_shader_control +
// blend_control[0] + ps_inputs. Excluded because they read GUEST MEMORY:
// Geometry (TryGetParams(regs.vs_program) for the GS copy shader) and Local
// (hull-stage tessellation constants). Export/Hull/TES excluded as
// rare-in-GR2 for a simpler proof; Compute never passes ctx_stable.
...
const GraphicsPipeline* PipelineCache::GetGraphicsPipeline(
    ...
    // Level 2.5 (GR2FORK, pipelineGfxKeyCtxSkipEnable, default-on): the key is
    // dirty, but only a user_data-only SH re-emit dirtied it -- no key-affecting
    // CONTEXT register changed (gfx_key_ctx_dirty is false). Every context-derived
    // field of the pipeline key is therefore identical to the committed key, so the
    // only thing that can differ is the resolved module set. Resolve the stages
    // (cheap now via the spec-fp cache) into graphics_key, leaving the cached
    // context fields intact, then run the same full-key memcmp the level-3 dedup
    // uses: if it still matches, the whole key is unchanged and we reuse the
    // pipeline without RefreshGraphicsKey's context-register rebuild (~0.89% on
    // GR2). A genuine module change fails the memcmp and falls through to the full
    // rebuild below, which re-resolves stages and rebuilds the now-stale context
    // fields. Gated on IsVertexInputDynamicState() because vertex_buffer_formats is
    // a user_data-derived key field only when vertex input is not dynamic.
    if (Config::isPipelineGfxKeyCtxSkipEnabled() && !regs.gfx_key_ctx_dirty &&
        last_gfx_pipeline && instance.IsVertexInputDynamicState()) [[likely]] {
        // GR2FORK STAGE-MEMO: this is the ONLY ctx_stable=true call site —
        // the snapshot just proved !gfx_key_ctx_dirty, which licenses the
        // per-stage resolve memo inside bind_stage/GetProgram.
        //
        // =====================================================================
        // GR2FORK K1 (2026-06-10): stage-region compare instead of the full-key
        // memcmp (~450 B, 0.37%sys measured under GetGraphicsPipeline in the
        // 2026-06-10 profile). The full memcmp was comparing bytes this gate
        // already proves equal: under !gfx_key_ctx_dirty every context-derived
        // key field is identical to the committed key (the original Level-2.5
        // proof above), and RefreshGraphicsStages(ctx_stable=true) mutates
        // EXACTLY three things (audited 2026-06-10, exhaustive `key.` grep over
        // the function): stage_hashes[*] (bind_stage, all paths), mrt_mask, and
        // num_color_attachments. Its only other key write —
        // vertex_buffer_formats — is gated on !IsVertexInputDynamicState(),
        // which this branch's own precondition excludes. ⚠ If that
        // precondition is ever relaxed, vertex_buffer_formats MUST join the
        // compare set. So comparing those three fields is equality-equivalent
        // to the full memcmp on this path; a genuine module/mrt change still
        // fails here and falls through to the full rebuild exactly as before.
        // Kill switch GR2_NOSTAGECMP restores the full memcmp verbatim.
        // =====================================================================
        if (RefreshGraphicsStages(regs, /*ctx_stable=*/true) &&
            (StageCmpNarrowEnabled()
                 ? (std::memcmp(graphics_key.stage_hashes.data(),
                                prev_graphics_key_.stage_hashes.data(),
                                sizeof(graphics_key.stage_hashes)) == 0 &&
                    graphics_key.mrt_mask == prev_graphics_key_.mrt_mask &&
                    graphics_key.num_color_attachments ==
                        prev_graphics_key_.num_color_attachments)
...
PipelineCache::LaunchAsyncPipelineCompile(const GraphicsPipelineKey& key, u64 pipeline_hash) {
    ...
    static const u64 compile_affinity_mask = []() -> u64 {
        // GR2FORK: GetHotCoreFenceMask returns 0 in non-exclusive mode, which
        // takes the forbidden==0 early-out below and leaves the compile worker
        // unconstrained — free to use the (idle) GpuComm/GpuAssembler cores
        // during a GRR compile burst on a 4-physical-core host.
...
bool PipelineCache::RefreshGraphicsKey(const AmdGpu::LiverpoolRegsSnapshot& regs) {
    ...
    // GR2FORK STAGE-MEMO: full-rebuild path — ctx may have changed, so the
...
bool PipelineCache::RefreshGraphicsStages(const AmdGpu::LiverpoolRegsSnapshot& regs,
    ...
    const auto bind_stage = [&](Shader::Stage stage_in, Shader::LogicalStage stage_out) -> bool {
        ...
        // PERF(GR2FORK STAGE-MEMO): under ctx_stable the program-address SH
        // words are VALUE-frozen since the last full resolve (any non-ud SH
        // write raises gfx_key_ctx_dirty), so when the memo's code pointer
        // still equals the live program address, TryGetParams would re-derive
        // exactly {same code span, same hash} — skip its 2-3 guest-memory
        // reads and reconstruct params from the memo. user_data is repointed
        // to the CURRENT snapshot (the one input allowed to change under
        // !ctx_dirty); GetProgram then flows it through the unchanged
        // downstream ladder. Soundness proof + inherited-exposure notes on
        // StageResolveMemo in vk_pipeline_cache.h.
        StageResolveMemo& memo = stage_memo_[stage_out_idx];
        if (ctx_stable && StageMemoEnabled() && memo.valid &&
        ...
        // GR2FORK: a non-null program pointer can still point at
        // mapped-but-non-shader memory (a stale pointer reused across pipelines,
        // or memory the guest streamed over before this async compile ran). The
        // recompiler's binary-info search would otherwise hit a fatal
        // UNREACHABLE; treat it like the null-address case above and skip the
        // stage, logging the address so a recurring real shader (deeper bug) is
        // distinguishable from a one-off stale pointer.
        if (!params_opt) [[unlikely]] {
            ...
            // GR2FORK STAGE-MEMO: the code at this address became unreadable
    ...
    // PERF(GR2FORK R2, 2026-06-10): per-call member-array reset. The previous
    // `infos.fill(nullptr); modules.fill(nullptr);` was auto-vectorized into
    // YMM stores + vzeroupper (stl_algobase.h:925), and annotate (capture
    // 2026-06-10-g) measured 80.46% of RefreshGraphicsStages SELF on that
    // vzeroupper (~0.8%sys at the site) - the retire point of the wide-store
    // drain. The scalar loops below keep identical semantics with plain
    // 8-byte stores; the per-iteration compiler barrier blocks loop- and
    // SLP-vectorization so the YMM/vzeroupper sequence cannot reappear.
    // Code-level change: A/B by build via the boot echo (no env switch can
    // alter codegen).
    {
        static const bool gr2_r2_echoed = []() {
            LOG_INFO(Render_Vulkan,
...
PipelineCache::Result PipelineCache::GetProgram(Stage stage, LogicalStage l_stage,
                                                ...
                                                const AmdGpu::LiverpoolRegsSnapshot& regs,
                                                bool ctx_stable) {
    // (GR2FORK STAGE-MEMO probe removed 2026-06-10 — investigation concluded:
    // measured ctx_stable = 66.6% of GetProgram calls, hit/stable = 100.0%
    // over 15.2M calls on the forced-gates build. Per the probe rule: each
    // live probe costs ~0.5% GpuAssembler; removed once its question is
    // answered.)

    // PERF(GR2FORK STAGE-MEMO): consult the per-stage resolve memo. On a hit
    // the BuildRuntimeInfo rebuild is skipped — runtime_infos[l_stage] still
    // holds the byte-identical last build (every register input is
    // value-frozen under ctx_stable; proof on the StageResolveMemo doc block
    // in vk_pipeline_cache.h) — along with HashRuntimeInfoForStage and the
    // program_cache map probe. params.hash equality self-validates the
    // program identity. Everything downstream is UNCHANGED: pgm_base /
    // user_data repoint, RefreshFlatBuf (incl. its SRT walker — deliberately
    // kept, preserving exact deployed semantics), the ud XXH3, the
    // last_result / ud_hash_lru / spec_fp tier ladder, AddBindings.
    StageResolveMemo& memo = stage_memo_[static_cast<u32>(l_stage)];
    const bool memo_hit = ctx_stable && StageMemoEnabled() && memo.valid &&
    ...
    if (memo_hit) {
        ...
        if (new_program) [[unlikely]] {
            ...
            auto start = binding;
            // PERF(GR2FORK STAGE-MEMO const-ref): BuildRuntimeInfo returns a
            // const& into runtime_infos[l_stage]; the old code copied it by
            // value at function entry, which also (silently) shielded the
            // member from the RuntimeInfo MUTATION CompileModule performs for
            // tess stages (TranslateProgram -> TessellationPreprocess writes
            // runtime_info.hs_info/vs_info.InitFromTessConstants — see
            // hull_shader_transform.cpp:152-154). Preserve those semantics
            // byte-for-byte: compile against a discardable local copy. The
            // mutation stays visible to the spec built right after it
            // (exactly as before, where the spec read the post-mutation local
            // copy); the member keeps the pristine pre-mutation build. Cold
            // [[unlikely]] path — the ~200 B copy is free here.
            ...
            // PERF(GR2FORK STAGE-MEMO): populate here too, so the very next
            // ctx-stable draw of a freshly compiled shader already hits.
            // ri_hash_raw is hashed from the MEMBER (pre-mutation build) —
            // that is the object a Level-2.5 hit reuses.
        ...
        // PERF(GR2FORK STAGE-MEMO): single steady-state populate site, placed
        // BEFORE the tier ladder's early returns so every exit path leaves
        // the memo current. On memo_hit no repopulate is needed — the values
    ...
    const bool spec_fp_lru_enabled = Config::isPipelineSpecFpLruEnabled();
    u64 ud_hash;
    u64 ri_bind_hash;
    // GR2FORK spec-fingerprint LRU state. spec_fp is computed in the FP tier
    // below (on the ud_hash-miss path) and reused at the populate site before
    // the final return. HS/DS are excluded: their spec folds tessellation
    // constant-buffer CONTENTS the descriptor-only key cannot cover.
    u64 spec_fp = 0;
                                  ...
                                  l_stage != LogicalStage::TessellationEval;
    {
        ...
        // PERF(GR2FORK STAGE-MEMO): ri_hash_raw is HashRuntimeInfoForStage of
        // runtime_infos[l_stage] — recomputed above on the miss path, served
        // from the memo on a hit (the member is byte-identical to a rebuild
        // under ctx_stable, so the hash is too). The binding fold below stays
        // LIVE either way, so a divergent binding state self-invalidates the
        // ud/last_result tiers instead of aliasing them.
        ...
        // PERF(GR2FORK): spec-fingerprint LRU resolution tier. Address-
        // independent permutation resolver -- catches the GR2 per-draw
        // UBO-pointer churn that ud_hash_lru structurally cannot (ud_hash is
        // XXH3 of the raw user_data bytes, which change every draw; the spec
        // result does not -- the probe measured a 100% redundant-respec
        // ceiling). See Program::spec_fp_lru for the safety argument and the
        // 0-collision/12.2M validation. The fetch-shader layout is pgm-static,
        // so the key reads it from any existing permutation (modules[0])
        // rather than re-parsing. On a hit this skips the StageSpecialization
        // construct below exactly as ud_hash_lru does, and re-resolution is
        // NOT suppressed (we are still inside GetProgram, recomputing the key,
        // and BindResources still writes the live address downstream).
        if (spec_fp_eligible && !program->modules.empty()) {
            // PERF(GR2FORK STAGE-MEMO ri-fp): reuse the memoized raw-byte
            // RuntimeInfo hash when the member is provably untouched
            // (memo_hit), compute-and-store it otherwise. Populate sites
            // reset ri_fp_hash to 0 whenever runtime_info is rebuilt, so a
            // nonzero memo value always corresponds to the CURRENT member
            // bytes — reuse is exact, not approximate.
            u64 ri_fp_hash;
    ...
    if (!found) {
        ...
        if (found_idx == std::numeric_limits<size_t>::max()) {
            ...
            // PERF(GR2FORK STAGE-MEMO const-ref): see the new-program path —
            // CompileModule may MUTATE its RuntimeInfo& (tess preprocess);
            // feed it a discardable copy. Matches the old by-value semantics
            // exactly: here the spec was ALREADY constructed above from the
            // pre-mutation state, and nothing after this call reads
            // runtime_info, so the mutation is discarded as before. Cold
            // [[unlikely]]-class path (new permutation) — the copy is free.
            Shader::RuntimeInfo ri_compile = runtime_info;
    ...
    // PERF(GR2FORK): record this draw's spec fingerprint -> resolved perm_idx
    // so the next draw with structurally identical (address-masked) sharps
    // skips the StageSpecialization construct above. All three resolution
    // outcomes (sig-map hit, linear-scan hit, new permutation) converge here
    // with perm_idx finalized. spec_fp is nonzero only when it was computed in
    // the eligible FP tier above (HS/DS and the disabled build leave it 0).
