...
void FsrPass::ResizeAndInvalidate(u32 width, u32 height) {
    // GR2FORK FIX: on an output-size change the old-size vk::Images must be released before
    // CreateImages runs again - UniqueImage::Create() has an ASSERT(!image) that aborts on a
    // stale handle (the GR2 ultrawide + FSR crash), and UniqueImage's move-assign leaks the old
    // image/allocation, so free via vmaDestroyImage directly. waitIdle first: FSR's per-Img
    // cycle is independent of the presenter's per-frame fence, so an in-flight frame may still
    // sample an Img about to be freed. Resizes are rare, so a full device sync is acceptable.
