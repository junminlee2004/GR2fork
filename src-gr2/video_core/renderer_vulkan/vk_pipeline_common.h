...
public:
    ...
    // FIX(GR2FORK): null-safe stage accessor for the device-fault breadcrumb
    // trail (diag builds). Unlike GetStage, it never dereferences a null stage
    // slot — a graphics pipeline legitimately has no fragment stage (depth-only
    // / shadow passes), and the breadcrumb record must read the fragment hash
    // without crashing on those. Returns nullptr when the stage is absent.
    // Returns a pointer (not a hash) so the forward-declared Shader::Info type
    // is only completed at the call site, keeping this header dependency-free.
...
// GR2FORK L2-PROBE: STRIPPED (2026-06-10, cleanup pass). The census it ran
// is answered and permanent: 89 distinct layout shapes / 2126 pipelines.
// The D2 lever built on that census (shape-shared layouts + relaxed
// desc-delta epoch key) was measured as a REGRESSION on 2026-06-10 and is
// kill-listed; layouts stay per-pipeline.
