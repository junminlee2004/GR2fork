...
void Scheduler::BeginRendering(const RenderState& new_state) {
    ...
    EndRendering();
    is_rendering = true;
    // PERF(GR2FORK R1): trimmed commit copy — active span only. Stale bytes
    // in inactive slots are never read (colorAttachmentCount below) and the
    // active-field operator== above never compares them. Full assignment
    // restored verbatim under GR2_NORSTRIM=1.
...
void Scheduler::AllocateWorkerCommandBuffers() {
    ...
    // Cross-pipeline cmdbuf state caches are scoped to the current cmdbuf —
    // a fresh cmdbuf has no pushed/bound state, so all per-Pipeline caches
    // must be considered invalid. Bumping the gens ensures every Pipeline's
    // cached_*_gen comparison fails on the first BindResources call against
    // this cmdbuf.
...
void Scheduler::SubmitExecution(SubmitInfo& info) {
    ...
    // FIX(GR2FORK): on VK_ERROR_DEVICE_LOST, query VK_EXT_device_fault BEFORE
    // the ASSERT fires, otherwise assert_fail_impl() runs Common::Log::Stop()
    // and `int 3`s into the SEH handler with no GPU-side detail in the log.
    // After this dump every DEVICE_LOST crash carries fault address(es),
    // access type, and any vendor-specific code/data that VK_EXT_device_fault
    // produces. The ASSERT itself is unchanged — the device is dead, the
    // process must still terminate, but now the log says WHY.
...
// FIX(GR2FORK): VK_EXT_device_fault diagnostic helper. Two-pass query per
// the spec: first call with pFaultInfo == nullptr returns the counts; then
// allocate per-count storage and re-call to fill it. We do NOT request the
// vendor binary blob — vk_instance.cpp creates the device with
// `deviceFaultVendorBinary = false`, so requesting it here would fail or
// return an empty buffer anyway; setting `vendorBinarySize = 0` and
// `pVendorBinaryData = nullptr` on the second pass keeps that consistent.
//
// All log lines are LOG_CRITICAL so they survive the Common::Log::Stop()
// flush that ASSERT_MSG triggers immediately after this helper returns.
// Wrapped in a single try-equivalent guard: anything that throws here would
// be worse than the original blank assert, so we use `noexcept` and check
// every Vulkan result directly. If the device-fault extension isn't loaded
// or the runtime call fails, we say so and return — the ASSERT still fires
// with "Device lost during submit" exactly as before.
void Scheduler::DumpDeviceFaultInfo(const Instance& instance, const char* context) noexcept {
    ...
    // `description` is a fixed-size char buffer (vk::ArrayWrapper1D<char,
    // VK_MAX_DESCRIPTION_SIZE>); cap by ::strnlen so a missing NUL doesn't run
    // off the end. Use .data() explicitly — ArrayWrapper1D's implicit-to-ptr
    // conversion is not consistent across vulkan-hpp revisions.
