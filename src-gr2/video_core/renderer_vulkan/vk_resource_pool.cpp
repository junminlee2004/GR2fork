...
std::size_t ResourcePool::CommitResource() {
    ...
    // GR2FORK PERF: direct check on the hint slot. Round-robin reuse means the first probe
    // overwhelmingly hits it, and routing that case through the lambda + std::optional search
    // below costs a construct/destruct per commit; fast-path it as a single branch.
...
CommandPool::CommandPool(const Instance& instance, MasterSemaphore* master_semaphore,
    ...
    // for call sites that don't specify a queue family.
    const u32 family =
...
vk::DescriptorSet DescriptorHeap::Commit(vk::DescriptorSetLayout set_layout) {
    ...
    // GR2FORK PERF: once warmed, each set_layout has a batch of sets pre-allocated and the
    // empty case fires only on exhaustion - rare per draw. Hint the cache-hit return.
    ...
    // GR2FORK PERF: allocation success is the dominant path; pool
