...
void Emulator::Run(std::filesystem::path file, std::vector<std::string> args,
    ...
    LOG_INFO(Loader, "Build: v4.9.004");
    ...
    if (Common::g_is_release) {
        if (remote_host == "shadps4-emu" || remote_url.length() == 0) {
            window_title = fmt::format("Junmin Lee GR2FORK v4.9.004 (v{}) | {}", Common::g_version,
                                       game_title);
        } else {
            window_title = fmt::format("Junmin Lee GR2FORK v4.9.004 {}/(v{}) | {}", remote_host,
                                       Common::g_version, game_title);
        }
    } else {
        if (remote_host == "shadps4-emu" || remote_url.length() == 0) {
            window_title = fmt::format("Junmin Lee GR2FORK v4.9.004 (v{}) {} {} | {}",
