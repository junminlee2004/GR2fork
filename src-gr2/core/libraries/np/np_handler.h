...
// GR2FORK: the score/leaderboard slice of upstream's shadNet NpHandler. Gravity Rush
// Remastered's online is PSN leaderboards only, served by the public shadNet server, so the
// handler carries the connection lifecycle and the sceNpScore dispatch verbatim from upstream
// and omits the friends/matching/TUS/invitation subsystems. Activation is gated to GRR serials;
// GR2 titles keep the restoration server path and never start a session here.
class NpHandler {
public:
    ...
    // Probe the server and connect the local user when shadNet is enabled for this title.
    void Initialize();

    // Disconnect, stop the worker thread, fail pending requests.
    void Shutdown();
    ...
    /// Full NP ID for this user, built once from the configured Online ID after login.
    ...
    // Reverse lookup: server account_id to local user_id. Returns -1 if no connected user
    // owns that account_id.
...
private:
    ...
    // True when this title should use shadNet: a GRR serial, credentials configured, the
    // network/PSN toggles on, and the session not disabled by a failed handshake.
    bool ShadNetEnabled() const;
    ...
    // Connect the local user from the configured credentials.
    ...
    // Fail every pending score request submitted by this user so pollers and blocked
    // WaitAsync callers wake up instead of hanging forever.
    ...
    // General async reply dispatch. Routes score replies; called from the per-user
    ...
    // Appear-Offline preference, applied at login.
    std::atomic<bool> m_appear_offline{false};

    // Session disabled for this run by an incompatible-protocol handshake.
    std::atomic<bool> m_session_disabled{false};

    // Per-user client map
    mutable std::mutex m_mutex_clients;
    std::map<s32, std::shared_ptr<ShadNet::ShadNetClient>> m_clients;
    // Per-user NP ID built once from the configured Online ID after login.
