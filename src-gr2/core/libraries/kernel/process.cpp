...
s32 PS4_SYSV_ABI sceKernelIsNeoMode() {
    // GR2FORK PERF: both inputs are fixed for process lifetime (config is set at launch, PSF
    // support_neo_mode at ELF load, before any call site can fire), and hot paths (ImageInfo
    // alt_tile detection, gnmdriver draws) call this enough to show at 0.05% of DrawThread.
