...
// IPMI kernel-level message router, RE'd from the libSceIpmi.sprx PRX wrappers. On real PS4,
// ipmimgr_call is a kernel syscall routing IPC messages between client/server endpoints; the
// output is always a simple s32 kernel ID (KID), never a C++ object pointer.
...
int PS4_SYSV_ABI ipmimgr_call(int opcode, int sub_id, void* output_ptr,
    ...
    // InvokeAsyncMethod, called by the gallery for photo data delivery. PRX wrappers: 0x4cc0
    // (opcode 0x253, 0x20-byte input with a size field) and 0x4d80 (opcode 0x254, 0x18-byte
    // input without one).
