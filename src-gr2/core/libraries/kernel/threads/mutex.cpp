...
// GR2FORK: match condvar/EF/sema slow-wait threshold (500ms).
...
static int MutexInit(PthreadMutexT* mutex, const PthreadMutexAttr* mutex_attr, const char* name) {
    ...
    if (name) {
        ...
        static std::atomic<int> MutexId{0};
...
int PthreadMutex::Lock(const OrbisKernelTimespec* abstime, u64 usec) {
    ...
    if (m_protocol == PthreadMutexProt::None) [[likely]] {
        ...
        // GR2FORK: no yield loop here - with PTHREAD_ADAPTIVE_MUTEX backing m_lock, the kernel
        // already spins adaptively in m_lock.lock(), so yielding only adds sched_yield() calls.
    }

    // GR2FORK: record kernel-fallback mutex contention (the spin fast path above is not
    // recorded). Without this, a thread blocked on a contested mutex is invisible in hang dumps.
    // arg1 = owner pointer at wait-start, identifying which holder needs investigating.
    ...
    // GR2FORK: record slow exits - successful lock after long contention,
