...
// GR2FORK: match event_flag.cpp threshold so condvar slow-waits are
...
int PthreadCond::Wait(PthreadMutexT* mutex, const OrbisKernelTimespec* abstime, u64 usec) {
    ...
    // GR2FORK: make condvar waits visible to SyncTrace - guest pthread_cond_wait parks on a
    // per-thread semaphore that bypasses the SET_EF/SIGNAL_SEMA ring, hiding the deadlock root
    // in hang dumps. arg1 carries the mutex reacquired on wake, cross-referencing WAIT_MUTEX_SLOW.
    ...
    for (;;) {
        ...
        if (curthread->wchan == nullptr) {
            ...
            mp->CvLock(recurse);
            // GR2FORK: record cancel exit if the wait was long enough
            // to be interesting.
    ...
    // GR2FORK: record slow exits (normal wake or timeout) so post-mortem
...
int PthreadCond::Signal(Pthread* thread) {
    ...
    // GR2FORK: record the signal only when a waiter was present - the common no-waiters fast
    // path would flood the SyncTrace ring and mask real signals before a hang. arg2=1 means
    // "single waiter woken".
    Common::SyncTrace::Record(
...
struct BroadcastArg {
    ...
    int count;
    // GR2FORK: total waiters woken across all defer-batches and the
...
int PthreadCond::Broadcast() {
    ...
    // GR2FORK: record broadcast - only when waiters were actually woken.
