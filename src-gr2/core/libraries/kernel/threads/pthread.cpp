...
// GR2FORK PERF: platform headers for host thread-affinity calls.
#ifdef _WIN64
#include <windows.h>
...
static void RunThread(void* arg) {
    ...
    // GR2FORK PERF: fence new guest threads off the GpuComm/GpuAssembler reserved cores at
    // birth - Windows CreateThread does not inherit affinity, so a fresh thread could sit on a
    // hot core up to ~5 s until the periodic rewalk. GetExclusionStripMask no-ops with the walk.
...
int PS4_SYSV_ABI posix_pthread_create_name_np(PthreadT* thread, const PthreadAttrT* attr,
    ...
    if (ret != 0) {
        ...
        // user-supplied stack; Free recycles or releases the Pthread.
        *thread = nullptr;
...
int Pthread::SetAffinity(const Cpuset* cpuset) {
    ...
    // GR2FORK: kept as upstream's no-op - some games get performance problems when affinity is
    // applied. The host-side reservation (GpuComm/assembler self-pinning plus the exclusion walk
    // and its 5 s Windows rewalk) already keeps guest threads off the reserved cores.
    /*
