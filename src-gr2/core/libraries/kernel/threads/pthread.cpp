...
static void RunThread(void* arg) {
    ...
    // PERF(GR2FORK): fence this thread off the GpuComm/GpuAssembler reserved
    // cores AT BIRTH, applying exactly what the exclusion walk would impose
    // (see the affinity scheme comment in posix_pthread_setaffinity_np below:
    // the desired outcome is "guest threads end up on (full host CPUs) AND
    // NOT (reserved cores)"). GetExclusionStripMask carries the walk's own
    // gating, so this is a no-op in every mode where the walk no-ops
    // (non-exclusive, fallback-pool topologies, no reserved core).
    //
    // Why here: on Linux the walk + pthread_create inheritance already
    // produce this mask, making the call an idempotent re-apply. On Windows
    // CreateThread does NOT inherit thread affinity — every thread the game
    // spawns (Havok workers, AvPlayer demuxer/decoder Kernel::Threads, audio
    // threads) arrives with the FULL host mask and could land on a hot core
    // for up to ~5 s until StartPeriodicAffinityRewalk's next pass evicted
    // it. Fencing in the trampoline closes that window to zero for every
    // thread routed through pthread machinery. GpuComm and the assembler are
    // std::jthreads and never pass through here, so they cannot be fenced
    // off their own cores. The setter clips ~strip to real host CPUs
    // (host_cpus & ~strip), which can never be an empty mask.
...
int PS4_SYSV_ABI posix_pthread_rename_np(PthreadT thread, const char* name) {
    ...
    if (thread == g_curthread) {
        ...
        if (name && False(thread->attr.flags & PthreadAttrFlags::StackUser)) {
            VAddr stack_addr = reinterpret_cast<VAddr>(thread->attr.stackaddr_attr);
