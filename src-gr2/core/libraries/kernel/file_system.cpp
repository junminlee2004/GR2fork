...
s32 PS4_SYSV_ABI open(const char* raw_path, s32 flags, u16 mode) {
    ...
    // GR2FORK: the redirects below serve a host file that lives outside every mount, so the
    // mount stack cannot open it by guest name. Record the target and open it directly.
    std::optional<std::filesystem::path> gr2_host_redirect;

    // GR2FORK: gallery photo redirect. FIOS2 passes photo paths ("photo/CONTENT_ID.jpg",
    // "/app0/photo/...") that match no mount; retry the bare filename in the screenshots
    // directory, with and without .jpg since the exact name format is unknown.
    if (!exists && std::string_view{raw_path}.find("photo") != std::string_view::npos) {
        ...
        if (photo_pos != std::string_view::npos) {
            ...
            auto screenshots_dir =
                Common::FS::GetUserPath(Common::FS::PathType::ScreenshotsDir) / "GR2_PhotoApp_HLE";
            ...
            if (fs::exists(try_path)) {
                ...
                gr2_host_redirect = try_path;
                exists = true;
                read_only = true;
                LOG_INFO(Kernel_Fs, "[GR2v57] Photo redirect: '{}' -> '{}'", raw_path,
                         try_path.string());
            } else {
                LOG_WARNING(Kernel_Fs, "[GR2v57] Photo path not found: '{}' (tried '{}')", raw_path,
                            (screenshots_dir / filename).string());
            }
        }
    }
    ...
    if (std::string_view{raw_path}.find("photo") != std::string_view::npos) {
        LOG_INFO(Kernel_Fs, "[GR2v57] open() with photo path: '{}' exists={} host='{}'", raw_path,
                 exists, file->m_host_name.string());
    }
    ...
    if (std::string_view{raw_path}.find("j=") != std::string_view::npos) {
        LOG_INFO(Kernel_Fs, "[GR2v155x] j= file open: '{}' exists={} host='{}'", raw_path, exists,
    ...
    if (std::string_view{raw_path}.find("film_dummy") != std::string_view::npos) {
        ...
        if (gnf_mgr.IsReady()) {
            ...
            if (!photo_path.empty() && fs::exists(photo_path)) {
                ...
                gr2_host_redirect = photo_path;
                exists = true;
                read_only = true;
                LOG_INFO(Kernel_Fs, "[GR2v155] film_dummy.gnf -> {}", photo_path.string());
            }
        }
    }
    ...
    if (directory) {
        ...
        // GR2FORK: a redirected path is outside every mount, so serve it as a plain host file
        // instead of resolving the guest name through the mount stack.
        if (gr2_host_redirect) {
            file->handle = std::make_unique<Core::FileSys::HostFile>(
                *gr2_host_redirect, Common::FS::FileAccessMode::Read, /*read_only=*/true);
        } else {
...
// GR2FORK PERF: read straight into the guest buffer. Upstream stages every read through a
// thread_local bounce buffer to dodge a silent std::fread failure seen in some titles, which
// costs a full extra copy on each read and keeps a per-thread allocation as large as the
// biggest request. GR2 does not hit that failure and is read-heavy, so the copy is pure loss.
