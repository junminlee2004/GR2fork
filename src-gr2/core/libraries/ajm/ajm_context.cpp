...
void AjmContext::WorkerThread(std::stop_token stop) {
    ...
    // GR2FORK PERF: the AJM worker burns ~1.3% of system time on guest cores, preempting
    // MTTW/DrawThread mid-frame. Park it on the emulator-reserved GpuComm/assembler cores (idle
    // headroom for a 1.3% tenant); mask is 0 on small hosts - it floats. Kill: GR2_NOAJMPIN=1.
    static const bool ajm_pin = []() noexcept {
        const char* e = std::getenv("GR2_NOAJMPIN");
        return !(e && e[0] == '1');
    }();
    if (const u64 mask = Common::GetReservedCoreMask(); ajm_pin && mask != 0) {
