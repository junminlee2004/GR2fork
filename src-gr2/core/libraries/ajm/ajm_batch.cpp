...
AjmJob AjmJobFromBatchBuffer(u32 instance_id, AjmBatchBuffer batch_buffer) {
    ...
    while (!batch_buffer.IsEmpty()) {
        ...
        case Identifier::AjmIdentInputRunBuf: {
            ...
            // T5.E2: first chunk borrows guest memory; a second chunk (rare)
            // materializes the copy path. See AjmJob::Input::borrowed doc.
            static const bool span_enabled = []() noexcept {
                const char* e = std::getenv("GR2_NOAJMSPAN");
                return !(e && e[0] == '1');
            }();
            if (span_enabled && job.input.buffer.empty() && job.input.borrowed.empty()) {
                job.input.borrowed = std::span<u8>(p_begin, buffer.size);
            } else {
                if (!job.input.borrowed.empty()) {
                    job.input.buffer.assign(job.input.borrowed.begin(),
                                            job.input.borrowed.end());
                    job.input.borrowed = {};
                }
