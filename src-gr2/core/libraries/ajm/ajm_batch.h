...
struct AjmJob {
    struct Input {
        ...
        // GR2FORK PERF: the common single-InputRunBuf job borrows a span over guest memory
        // instead of a per-job heap memcpy; AJM owns the buffers until batch completion, so the
        // span stays valid. Multi-chunk or GR2_NOAJMSPAN=1 copies; read via Data() only.
        std::span<u8> borrowed{};

        [[nodiscard]] std::span<u8> Data() noexcept {
            return borrowed.empty() ? std::span<u8>(buffer) : borrowed;
        }
    };
