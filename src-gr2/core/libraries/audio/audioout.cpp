...
// [SNDMOD] for the title-theme bus-volume handoff to the direct player
...
int PS4_SYSV_ABI sceAudioOutGetLastOutputTime(s32 handle, u64* output_time) {
    // GR2FORK PERF: demoted from LOG_DEBUG - games poll this API;
    // LOG_DEBUG is compiled in and pays format-args + filter per call.
    LOG_TRACE(Lib_AudioOut, "called, handle: {}, output time: {}", handle, fmt::ptr(output_time));
    if (!output_time) {
...
s32 PS4_SYSV_ABI sceAudioOutOutput(s32 handle, void* ptr) {
    ...
    auto& port = ports_out.at(handle - 1);
    {
        ...
        if (ptr != nullptr && port.IsOpen()) {
            ...
            // [SNDMOD] Hand the BGM/main bus volume (set via sceAudioOutSetVolume) to the
            // title-theme direct player so a replacement track follows the game's level. Keyed
            // on the multichannel (>=6ch) bus; gated by the mod toggle (1.0 = 0 dB / full).
            if (Config::getGR2TitleThemeMod() && port.format_info.num_channels >= 6) {
                s32 maxv = 0;
                for (const s32 v : port.volume) {
                    if (v > maxv) {
                        maxv = v;
                    }
                }
                Ajm::TitleThemeMod::SetBusGain(static_cast<float>(maxv) /
                                               static_cast<float>(SCE_AUDIO_OUT_VOLUME_0DB));
            }
        }
    }
    return samples_sent;
