...
void PhotoTextureManager::InitFakeVtable(uintptr_t eboot_base) {
    ...
    // VA 0x374510 is a single `ret` (0xC3) - the stub the gallery widget's real vtable already
    // uses in slots 4-6 and 11 - so every vtable dispatch on the fake nodes returns harmlessly.
...
void PhotoTextureManager::SetupNode(std::vector<u8>& node) {
    ...
    // node+0xa0 points to sub_desc (offset 0x800 in this buffer). The per-frame scene-graph tick
    // at VA 0xfa5f60 reads four loop counts from it (+0xe0, +0xc8, +0xe8, +0xd8); all zeroed, so
    // all four loops skip safely.
