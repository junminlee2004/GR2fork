...
fs::path SaveInstance::MakeTitleSavePath(OrbisUserServiceUserId user_id,
    ...
    // GR2FORK: home-based layout, binary-compatible with the prerelease build:
    // <home>/<uid>/savedata/<serial>.
    return Config::GetHomeDir() / std::to_string(user_id) / "savedata" / game_serial;
}

fs::path SaveInstance::MakeDirSavePath(OrbisUserServiceUserId user_id, std::string_view game_serial,
                                       std::string_view dir_name) {
    return Config::GetHomeDir() / std::to_string(user_id) / "savedata" / game_serial / dir_name;
}
...
void SaveInstance::SetupAndMount(bool read_only, bool copy_icon, bool ignore_corrupt,
    ...
    if (!exists) {
        CreateFiles();
        if (copy_icon) {
            // GR2FORK FIX: read through the mount so archive-backed (.zar) games seed the icon.
