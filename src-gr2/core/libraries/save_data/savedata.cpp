...
static Error saveDataMount(const OrbisSaveDataMount2* mount_info,
    ...
    if (to_be_created) { // Check size
        ...
        // GR2FORK: home-based savedata root (matches MakeTitleSavePath /
        // the prerelease layout): <home>/<uid>/savedata.
        const auto root_save =
            Config::GetHomeDir() / std::to_string(mount_info->userId) / "savedata";
