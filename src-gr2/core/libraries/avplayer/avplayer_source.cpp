...
// GR2FORK FIX: extern declaration at global scope so the name resolves to the global-scope
// `presenter` defined elsewhere, not a nonexistent `Libraries::AvPlayer::presenter`. Same
// pattern as video_out.cpp and jpegenc.cpp.
...
bool AvPlayerSource::Start() {
    ...
    if (m_video_stream_index) {
        ...
        // GR2FORK FIX: FF_THREAD_FRAME stays off - GR2 cycles Stop/Start rapidly (28+ per
        // session) and freeing the codec context mid-decode races worker teardown (read AV at
        // +0x108 of a freed struct in avcodec-*.dll); 720p-max menu videos gain nothing from it.
    ...
    if (m_audio_stream_index) {
        ...
        // GR2FORK FIX: same reasoning as video - no threading in audio decode
...
bool AvPlayerSource::Stop() {
    ...
    // GR2FORK FIX: US disc (CUSA03694) hits VK_ERROR_DEVICE_LOST during newspaper comic
    // cutscenes - Stop() clears m_video_buffers before the GPU finishes sampling the last NV12
    // output. Drain all in-flight GPU work first; waitIdle runs only once per AvPlayer shutdown.
    if (::presenter) {
        ::presenter->WaitIdle();
    }
    ...
    // Always clean up resources, even when threads already self-terminated (e.g. after EOF).
    // On a real PS4 stopping a finished player is a successful no-op; returning false makes
    // AvPlayerState::Stop() skip its state transition, which can break the game's handle cycling.
    ...
    // GR2FORK FIX: tear down codec contexts with m_state_mutex held and decoder threads joined -
    // freeing at destruction or Start() reassignment races in-flight FFmpeg references (read AV
    // at +0x108 of a freed AVCodecContext); the scaler/resampler reference them, so reset too.
    m_sws_context.reset();
    m_swr_context.reset();
...
bool AvPlayerSource::GetVideoData(AvPlayerFrameInfoEx& video_info) {
    ...
    // GR2FORK FIX: recycle frames from a previous loop generation before the timestamp-based
    // selection below. A pre-seek frame's end-of-clip timestamp pins it at the head once the loop
    // reset zeroes the clock, backing up the queue until the decoder parks - silent loop hang.
    {
    ...
    // GR2FORK FIX: peek-then-decide via TryPeek (atomic under the queue's mutex), not unlocked
    // Size() + Front() - the DemuxerThread's loop-reset drain can empty the queue under the game
    // thread, leaving a dangling reference and a wild read on .info access.
...
bool AvPlayerSource::GetAudioData(AvPlayerFrameInfo& audio_info) {
    ...
    // GR2FORK FIX: atomic Pop with has_value check - Size()-then-Pop() is a TOCTOU against the
    // demuxer's loop-reset drain. Frames from an older loop generation are skipped so their
    // end-of-clip timestamps never reach m_last_audio_ts and stall GetVideoData's selection.
...
void AvPlayerSource::FlushFrozenFrames() {
    // Recycle the decoded backlog to the buffer pools: while suppressed the consumer stopped
    // draining, so the pools emptied and the decoders are likely parked on *_buffers_cv. The
    // queues are atomic - Pop/Push interleave safely with running decoders, no extra locking.
        ...
        if (res < 0) {
            if (res == AVERROR_EOF) {
                if (m_is_looping) {
                    ...
                    // Reset timing references so the new loop's timestamps (from 0) stay in sync
                    // with the wall clock; otherwise CurrentTime() drifts ahead each loop and
                    // the frame-skip logic drops increasingly more frames.
                    m_start_time = std::chrono::high_resolution_clock::now();
                    m_pause_duration = {};
                    m_last_audio_ts.reset();

                    // GR2FORK FIX: discard packets queued before the seek; decoded into the new
                    // generation they would carry end-of-clip timestamps. The epoch stamp covers
                    // the single in-flight straggler.
                    while (m_video_packets.Pop()) {
                    }
                    while (m_audio_packets.Pop()) {
                    }

                    // GR2FORK FIX: open the new loop generation after the stale packets are
                    // gone; an in-flight decoder frame came from an old-epoch packet, so the
                    // consumer recycles it instead of pinning it at the queue head.
                    m_loop_epoch.fetch_add(1, std::memory_order_acq_rel);

                    // GR2FORK FIX: while-pop pattern - Size()-then-Pop() is a TOCTOU against the
                    // game thread's GetVideoData path, which also pops from m_video_frames; an
                    // atomic Pop() returning nullopt cleanly terminates the drain loop.
                    while (auto old = m_video_frames.Pop()) {
                        m_video_buffers.Push(std::move(old->buffer));
                    }
                    m_video_buffers_cv.Notify();
                    ...
                    // GR2FORK FIX: FFmpeg requires single-thread access to an AVCodecContext;
                    // unlocked flush_buffers races the decoders' send_packet/receive_frame every
                    // loop iteration. Flush holds each mutex for only microseconds.
...
void AvPlayerSource::VideoDecoderThread(std::stop_token stop) {
    ...
    while ((!m_is_eof || m_video_packets.Size() != 0) && !stop.stop_requested()) {
        ...
        // GR2FORK FIX: sample the loop generation for this packet.
        ...
        // GR2FORK FIX: lock the codec context - the demuxer may call avcodec_flush_buffers on
        // the same context during a loop reset, and FFmpeg requires single-thread access.
        int res;
        {
            std::lock_guard lk(m_video_codec_mutex);
        ...
        while (res >= 0) {
            ...
            // GR2FORK FIX: lock codec context - see send_packet above.
            {
                std::lock_guard lk(m_video_codec_mutex);
...
void AvPlayerSource::AudioDecoderThread(std::stop_token stop) {
    ...
    while ((!m_is_eof || m_audio_packets.Size() != 0) && !stop.stop_requested()) {
        ...
        const u64 pkt_epoch = m_loop_epoch.load(std::memory_order_acquire);
        // GR2FORK FIX: lock codec context - same rationale as
