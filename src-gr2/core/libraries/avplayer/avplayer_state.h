...
public:
    ...
    bool IsActive();
    // GR2FORK FIX: forward a frozen-stream flush to the source. See
...
private:
    ...
    // GR2FORK FIX: Stop() body that assumes the caller holds m_source_mutex uniquely. Lets
    // Start() hold the unique lock across the whole Stop-then-Start transition without
    // deadlocking on a re-entrant shared-lock acquisition.
    bool StopLocked();
