...
struct Frame {
    ...
    // GR2FORK FIX: loop-epoch stamp (see m_loop_epoch). A frame decoded from a packet dequeued
    // before a loop-back seek keeps its end-of-clip timestamp, reads as "not yet due" forever
    // after the clock resets, and wedges m_video_frames; the consumer drops stale-epoch frames.
    u64 epoch{};
};
...
public:
    ...
    // GR2FORK FIX: recycle every decoded-but-unconsumed frame back to the buffer pool and wake
    // the decoders. A stream leaving tutorial-wins suppression has its decoder parked with the
    // pool drained into the frame queue; this unwedges it so playback resumes instead of freezing.
...
private:
    ...
    // GR2FORK FIX: loop generation, bumped by the demuxer on every loop-back seek. Decoders
    // stamp frames with the value sampled at packet dequeue; GetVideoData/GetAudioData discard
    // frames whose epoch is older.
    ...
    // GR2FORK FIX: serialize all send/receive/flush calls per codec context. FFmpeg requires
    // single-threaded AVCodecContext access; the demuxer's avcodec_flush_buffers() on loop reset
    // racing the decoder threads corrupts FFmpeg state and crashes during looping video.
