...
bool RecoverLostDevice() {
    // GR2FORK: VK_ERROR_DEVICE_LOST recovery (opt-in, [GPU] deviceRecovery). Rebuilds the whole
    // Vulkan graph with a fresh VkDevice by recreating the Presenter; the guest state (liverpool,
    // guest memory) survives and the buffer/texture caches repopulate on next access. Runs on the
    // VideoOut flip thread - never the assembler thread, never inside Present. Bounded so a
    // deterministic fault cannot loop forever; returns false at the session cap so the caller can
    // restore the fatal path.
    constexpr int kMaxDeviceRecoveries = 8;
    static std::atomic<int> attempts{0};
    const int n = attempts.fetch_add(1, std::memory_order_relaxed) + 1;
    if (n > kMaxDeviceRecoveries) {
        LOG_CRITICAL(Lib_GnmDriver,
                     "[DeviceRecovery] session cap ({}) reached - not rebuilding",
                     kMaxDeviceRecoveries);
        return false;
    }
    LOG_WARNING(Lib_GnmDriver,
                "[DeviceRecovery] device lost - rebuilding renderer (attempt {}/{})", n,
                kMaxDeviceRecoveries);
    // Teardown: ~Presenter drains the assembler via the Stage 1-defused waits, then destroys the
    // old (lost) device and every child object.
    presenter.reset();
    // Clear before the fresh graph so its new schedulers do not observe a stale loss.
    Vulkan::g_device_lost.store(false, std::memory_order_release);
    presenter = std::make_unique<Vulkan::Presenter>(*g_window, liverpool.get());
    LOG_WARNING(Lib_GnmDriver, "[DeviceRecovery] renderer rebuilt - resuming");
    return true;
}
...
void RegisterLib(Core::Loader::SymbolsResolver* sym) {
    ...
    // Drive PipelineCache::WarmUp here, after the Presenter is fully constructed, so it can
    // paint a "LOADING SHADERS" overlay on the swapchain while it deserializes. Large caches
    // take ~30s (41,896 cached pipelines on GR2), which otherwise looks like a black-window
    // hang since videoout's PresentThread starts only in a later RegisterLib.
