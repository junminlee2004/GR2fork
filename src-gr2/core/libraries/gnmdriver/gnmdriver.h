...
// GR2FORK: rebuild the renderer + VkDevice after VK_ERROR_DEVICE_LOST (opt-in, [GPU]
// deviceRecovery). Call from the VideoOut flip thread when Vulkan::g_device_lost is set. Returns
// false when the session recovery cap is hit (caller should restore the fatal path).
