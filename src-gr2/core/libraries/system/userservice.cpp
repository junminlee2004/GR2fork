...
// GR2FORK: the user service is now backed by the UserManager / UserSettings
// subsystem (users.json), matching the prerelease build. UserManagement
// (== UserSettings.GetUserManager()) owns the four default accounts 1000..1003
// and the logged-in slots; the default/initial user is UserManager::GetDefaultUser()
// (player_index 1 == user 1000). A login event for that user is queued at boot
// (UserSettings.Load() + the auto-login in main.cpp), so games observe one
// logged-in user exactly as before — only now persisted and routed through the
// account structure path_util provisions under <home>/<uid>/.

// Login/logout events produced by UserManager::LoginUser/LogoutUser, drained by
// sceUserServiceGetEvent below.
...
s32 PS4_SYSV_ABI sceUserServiceGetLoginUserIdList(OrbisUserServiceLoginUserIdList* userIdList) {
    ...
    // Initialize all slots to invalid (-1), then fill from the logged-in users.
    for (int i = 0; i < ORBIS_USER_SERVICE_MAX_LOGIN_USERS; i++) {
        userIdList->user_id[i] = ORBIS_USER_SERVICE_USER_ID_INVALID;
    }

    auto logged_in_users = UserManagement.GetLoggedInUsers();
    for (int i = 0; i < ORBIS_USER_SERVICE_MAX_LOGIN_USERS; i++) {
        userIdList->user_id[i] =
