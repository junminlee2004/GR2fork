...
// GR2FORK FIX: deliberately non-inlined re-fetch of the run context for the two post-longjmp
// resume sites. In the dlopened core the inlined GetFiberContext() can read a stale 0 from
// tcb_fiber (longjmp strands the cached general-dynamic TLS register); a fresh call re-resolves.
...
void PS4_SYSV_ABI _sceFiberCheckStackOverflow(OrbisFiberContext* ctx) {
    ...
    // GR2FORK: validate the fiber before trusting any field on it. An overflow can smash the
    // OrbisFiber itself (observed: garbage name, size_context 0x90), making the canary restore
    // below a wild write. Bail on inconsistency; prevention is Kernel::Thread::Run stack sizing.
    ...
    if (*stack_base != kFiberStackSignature) {
        // The canary sits at the lowest address of the fiber allocation, and HLE trampolines
        // use more host stack than native PS4 code, so on small game-allocated fiber stacks the
        // canary is touched without harm. Soft diagnostic: log once per fiber, restore, continue.
...
s32 PS4_SYSV_ABI sceFiberSwitchImpl(OrbisFiber* fiber, void* addr_context, u64 size_context,
    ...
    // GR2FORK FIX: an inlined GetFiberContext() read here can see a transient 0 from tcb_fiber
    // after the longjmp resume (see GR2_ResumeRefetchFiberContext), so resolve through the
    // noinline helper. The guard below is a tripwire that bails instead of dereferencing null.
...
s32 PS4_SYSV_ABI sceFiberReturnToThread(u64 arg_on_return, u64* arg_on_run) {
    ...
    if (cur_fiber->addr_context) {
        ...
        s32 jmp = _sceFiberSetJmp(&ctx);
        if (jmp) {
            // GR2FORK FIX: same stale-inlined-TLS re-fetch hazard as sceFiberSwitchImpl -
            // resolve through the noinline helper so the post-longjmp read happens fresh.
            // Guard is a tripwire.
            g_ctx = GR2_ResumeRefetchFiberContext();
            if (!g_ctx) {
                LOG_ERROR(Lib_Fiber,
