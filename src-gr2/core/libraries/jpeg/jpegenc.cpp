...
// GR2 Photo Mode: after HLE encode, the game's photo state machine needs its scene node version
// bumped and result-ready flag set - side-effects of the PS4 hardware JPEG encoder.
...
namespace {
    ...
    // GR2 Photo Mode: the game renders the photo to a GPU render target and calls
    // sceJpegEncEncode with a CPU pointer, so pixels are downloaded synchronously via a one-shot
    // Vulkan command buffer that bypasses the scheduler and Liverpool thread (avoids deadlocks).
    ...
    // ForceDownloadByAddress works independently of readbackLinearImages; enabling that flag
    // during encode makes the async pipeline race the synchronous copy and overwrite clean data.

    // GR2 Photo Mode host autosave: the game never calls ContentExport after sceJpegEncEncode,
    // leaving the gallery empty, so the encoded JPEG is written to the folder ContentSearch
    // already scans (<ScreenshotsDir>/GR2_PhotoApp_HLE/). Disable: SHADPS4_GR2_PHOTO_AUTOSAVE=0.
...
s32 PS4_SYSV_ABI sceJpegEncCreate(const OrbisJpegEncCreateParam* param, void* memory,
    ...
    // Zero-fill the entire memory buffer first - the real PS4 encoder populates encoding tables
    // and metadata here, and the game may read fields that are not explicitly set after encode.
...
s32 PS4_SYSV_ABI sceJpegEncEncode(OrbisJpegEncHandle handle, const OrbisJpegEncEncodeParam* param,
    ...
    // Snapshot the pixel data. After a successful ForceDownloadByAddress the data is stable and
    // a single memcpy suffices; on failure (no tracked RT) retry-based torn-read protection
    // guards against a concurrent async writeback.
    ...
    // The hardware encoder writes the actual JPEG output size back to param->jpeg_size; the game
    // passes output_info=NULL and reads it from there, so leaving the original buffer size in
    // place reads as an encode failure.
    {
    ...
    // On real PS4, sceJpegEncEncode returns the JPEG output size (positive s32) on success and a
    // negative error code on failure; returning ORBIS_OK (0) reads as "zero bytes encoded" and
    // the game shows the red placeholder.
