...
// GR2 upload probe: driven per-frame so we capture the submit-time pre-check state.
...
s32 PS4_SYSV_ABI sceVideoOutSubmitFlip(s32 handle, s32 bufferIndex, s32 flipMode, s64 flipArg) {
    ...
    Libraries::Np::NpAuth::Gr2ArcProbe("flip");
    return ORBIS_OK;
}
...
s32 sceVideoOutSubmitEopFlip(s32 handle, u32 buf_id, u32 mode, u32 arg, void** unk) {
    ...
    Platform::IrqC::Instance()->RegisterOnce(
        Platform::InterruptId::GfxFlip, [=](Platform::InterruptId irq) {
            ...
            // GR2FORK FIX: drop a late/out-of-order flip - a sibling EOP flip already presented
            // and zeroed this buffer's label (repro: AvPlayer cutscene skip); submitting the late
            // buf_id clobbers a pending frame's label and deadlocks BG fibers on EF bit 0x10000000.
    ...
    Libraries::Np::NpAuth::Gr2ArcProbe("eopflip");
    return ORBIS_OK;
}
