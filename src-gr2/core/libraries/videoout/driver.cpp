...
void VideoOutDriver::Flip(const Request& req) {
    // GR2FORK: device-lost recovery (opt-in, [GPU] deviceRecovery). Rebuild the renderer before
    // presenting. req.frame belongs to the destroyed presenter, so skip presenting it this flip
    // (one dropped frame - the hitch), but still complete the flip below so the guest keeps running.
    bool skip_present = false;
    if (Vulkan::DeviceRecoveryEnabled() &&
        Vulkan::g_device_lost.load(std::memory_order_acquire)) [[unlikely]] {
        if (!Libraries::GnmDriver::RecoverLostDevice()) {
            ASSERT_MSG(false, "[DeviceRecovery] recovery exhausted - device permanently lost");
        }
        skip_present = true;
    }
