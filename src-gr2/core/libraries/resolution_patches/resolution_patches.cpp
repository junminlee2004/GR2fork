...
enum class Kind {
    ...
    Exposure2_f,
    // ── Q1 additions (negative viewport-center half-dims) ───────────
    // Sign-complement of HalfW_f/HalfH_f. P1 scales the POSITIVE
    // (960,540) screen-center constants; these encode the NEGATIVE
    // forms (-960,-540) used as additive centering offsets in
    // screen = (ndc*scale) + (-halfDim) transforms (vaddps/vaddss
    // [rip+disp]). P1's positive-only encoders cannot reach them.
    // Baseline: -960.0f = 0xC4700000, -540.0f = 0xC4070000.
    NegHalfW_f,
    NegHalfH_f,
};
...
enum class Group : std::uint8_t {
    ...
    Q1,  // Negative viewport-center half-dims — the sign-complement of
         // P1. 7 rodata floats (-960/-540) consumed by screen-center
         // additive transforms (vaddps/vaddss) in fns ~0x135d000 /
         // 0x136e6e5 / 0x1364d1c / ~0x136494c. ordinal MUST be 31 to
         // match PatchGroupBit::Q1 = 1u<<31. Reachable ONLY via an
         // explicit `q1` token (e.g. resolutionPatchGroups =
         // "recommended,q1") — intentionally NOT in `all`, so the
         // hardware-unverified group never applies during routine
         // bisection or the `all`/`default` preset.
    Count
};
...
constexpr std::array<ResSite, 540> kResSites = {{
    ...
    // ── v18 Q1: negative viewport-center half-dims (P1 complement) ──
...
//       add E1   (I1/I2/I3/I4 REMOVED in v18 — they corrupt the
//                 position of screen-space effects at non-1080p)
...
//       add nothing   (I1/I2/I3/I4 REMOVED in v18; E1 also stays OFF —
//                      at sub-1080p the desired behavior is text that
//                      shrinks proportionally with the framebuffer)
//
//   * target H == 1080p:
...
constexpr std::uint32_t kTextScalingExtras_AboveDesign =
    ...
    // v18: I1/I2/I3/I4 removed — at non-1080p their UI-projection-lock /
    // ortho / viewport-scale constants offset screen-space effects
    // (e.g. the gravity-field overlay) away from their world anchor.
    // Only E1 (keeps text pixel-perfect) is auto-added above design res.
      static_cast<std::uint32_t>(PatchGroupBit::E1);

constexpr std::uint32_t kTextScalingExtras_BelowDesign =
    // C2/C3 + I1/I2/I3/I4 excluded from recommended entirely (v18).
