...
// sceLibcMspace: host-delegated allocator for PS4 mspace (dlmalloc) heaps - GR2 makes >10,000
// free/realloc-heavy allocations on its main mspace, beyond a bump allocator, so allocations come
// from the host heap (shared address space); the handle sits in a header at the region start.
...
int PS4_SYSV_ABI sceLibcMspacePosixMemalign(void* msp, void** memptr,
    ...
    // The game's primary aligned allocator - 10,295 call sites in GR2's eboot. Returns 0 on
    // success (writes the allocated pointer to *memptr), ENOMEM on failure.
...
size_t PS4_SYSV_ABI sceLibcMspaceMallocUsableSize(void* ptr) {
    ...
    // Single-argument, confirmed by RE of 21,338 call sites in GR2's eboot (only rdi is set).
    // Despite the name it works on the allocation's metadata, like dlmalloc_usable_size.
