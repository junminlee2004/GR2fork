...
s32 Linker::LoadModule(const std::filesystem::path& elf_name, bool is_dynamic) {
    ...
    // GR2FORK FIX: prefer opening through the mount so archive-backed (.zar) games load;
    // a guest path has no host file behind it. Host paths fall back to a direct open.
