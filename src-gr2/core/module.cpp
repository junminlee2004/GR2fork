...
void Module::LoadModuleToMemory(u32& max_tls_index) {
...
#if defined(ARCH_X86_64) && defined(_WIN32)
                // Windows static guest red-zone protection: function boundaries come from the
                // EH frame search table; without them nothing can be protected.
                if (use_static_windows_guest_red_zone_protection &&
    ...
    if (MemoryPatcher::g_eboot_address == 0) {
        if (name == "eboot.bin") {
            ...
            // Aspect-ratio override: runtime patch of the hard-coded 1.7777778f sites, applied
            ...
            // GR2 resolution override: runtime patches for the 37 hard-coded 1920x1080 constants
            // (groups in resolution_patches.cpp), composed with the aspect override. Driven by
            // [GPU] resolutionOverride + resolutionPatchGroups; falls back to GRR constants.
