...
// GR2FORK PERF: the size < 3_GB fast path is inlined in memory.h; this body is only reached for
// large ranges that may straddle multiple VMAs. The short-circuit stays as a defensive check for
// direct callers.
...
void MemoryManager::CopySparseMemory(VAddr virtual_addr, u8* dest, u64 size) {
    ...
    // GR2FORK: boot echo, once on first use.
    ...
    while (size) {
        ...
        if (vma->second.IsMapped()) {
            // GR2FORK PERF: streaming stores above the size threshold;
            // plain std::memcpy below it or under GR2_NONTCOPY=1.
