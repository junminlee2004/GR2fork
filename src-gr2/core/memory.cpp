...
// =============================================================================
// PERF(GR2FORK L6, 2026-06-10): non-temporal upload copy.
//
// CopySparseMemory is the guest-RAM -> staging-buffer copy under
// BufferCache::UploadCopies (+ the InlineData path). The 2026-06-10 profile
// puts its memcpy at ~2.7%sys of GpuAssembler (the ObtainBuffer memmove
// slice) — ~14.4M copies / ~18GB per session (B1 finals), avg ~1.25KB with a
// heavy byte tail. The staging data is written once by the CPU and consumed
// by the GPU; pulling it through the cache hierarchy evicts hot assembler
// working set for zero reuse. Streaming (non-temporal) stores skip the RFO
// and the cache fill for copies large enough to be tail traffic.
//
// Mechanics: SSE2 _mm_stream_si128 (x86-64 baseline — compiles without ISA
// flags on both boxes: Ally X znver4 and Deck znver2). Head bytes bring dest
// to 16B alignment with a plain copy; body streams 64B (one cache line / WC
// line) per iteration with unaligned loads (guest source has no alignment
// guarantee); tail is a plain copy. One _mm_sfence() per CopySparseMemory
// call (not per run) — required before another agent may observe the data;
// the actual GPU consume is fenced much later at submit, so this is
// conservative.
//
// Switches:
//   GR2_NONTCOPY=1   -> plain std::memcpy path, verbatim pre-L6 behavior.
//   GR2_NTCOPY_MIN=N -> streaming threshold in bytes (the pinned A/B plan is
//                       2048 first, then 1024 if flat — env-tunable so the
//                       second A/B needs no rebuild). Default 2048.
// FORCED ON — no longer config-controlled; the GR2_NO* env kills are the only
// way to disable/retune. A/B on BOTH boxes (coherency/cache-size sensitive),
// three-strikes.
// =============================================================================
namespace {
...
bool NtCopyEnabled() noexcept {
    static const bool enabled = []() noexcept {
        const char* e = std::getenv("GR2_NONTCOPY");
        return !(e && e[0] == '1');
    }();
    return enabled;
}

u64 NtCopyMinBytes() noexcept {
    static const u64 min_bytes = []() noexcept -> u64 {
        if (const char* e = std::getenv("GR2_NTCOPY_MIN")) {
            const u64 v = std::strtoull(e, nullptr, 10);
            if (v >= 64) {
                return v;
            }
        }
...
bool CopyMaybeNT(u8* dest, const u8* src, u64 size) noexcept {
    ...
    // Head: bring dest to 16B alignment (size >= threshold >= 64, no underflow).
    const u64 head = (16 - (reinterpret_cast<uintptr_t>(dest) & 15)) & 15;
    if (head) {
    ...
    // Body: one cache line per iteration; loads unaligned, stores streamed.
    while (size >= 64) {
        ...
        _mm_stream_si128(reinterpret_cast<__m128i*>(dest + 16), b);
        _mm_stream_si128(reinterpret_cast<__m128i*>(dest + 32), c);
        _mm_stream_si128(reinterpret_cast<__m128i*>(dest + 48), d);
        src += 64;
        dest += 64;
        size -= 64;
    }
...
void MemoryManager::CopySparseMemory(VAddr virtual_addr, u8* dest, u64 size) {
    ...
    // PERF(GR2FORK L6): boot echo, once on first use.
    static const bool gr2_l6_echoed = []() {
        LOG_INFO(Kernel_Vmm,
                 "[GR2FORK L6] NT-store upload copy: {} (forced on; GR2_NONTCOPY), "
                 "threshold {} bytes (GR2_NTCOPY_MIN)",
                 NtCopyEnabled(), NtCopyMinBytes());
        return true;
    }();
    (void)gr2_l6_echoed;
    ...
    while (size) {
        ...
        if (vma->second.IsMapped()) {
            // PERF(GR2FORK L6): streaming stores above the size threshold;
            // verbatim std::memcpy below it or under GR2_NONTCOPY=1.
            streamed |= CopyMaybeNT(dest, std::bit_cast<const u8*>(virtual_addr), copy_size);
        } else {
