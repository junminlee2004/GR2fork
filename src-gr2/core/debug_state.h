...
class DebugStateImpl {
    ...
    // GR2FORK PERF: advisory mirror of !waiting_reg_dumps.empty(), maintained at the two
    // mutation sites under the exclusive lock. Lets DumpingCurrentReg's per-draw fast path skip
    // the shared_lock RMWs when no dump is armed.
    std::atomic<bool> has_waiting_reg_dumps{false};
...
public:
    ...
    bool DumpingCurrentReg() {
        // GR2FORK PERF: called on every draw/dispatch on GpuComm; the shared_lock costs two
        // lock-prefixed RMWs just to test emptiness. Fast-path on the advisory flag; the locked
        // re-check preserves the exact locked semantics when a dump is armed.
        if (!has_waiting_reg_dumps.load(std::memory_order_relaxed)) [[likely]] {
            return false;
        }
