...
std::filesystem::path MntPoints::GetHostPath(std::string_view path, bool* is_read_only,
    ...
    // For the mount root itself (e.g. /app0), return the patch root when a patch applies so
    // iteration sees update content; resolving it to the base path makes IterateDirectory hit the
    // same base path for both its base and patch lookups, hiding top-level update files.
