...
bool UserManager::AddUser(const User& user) {
    ...
    const auto user_dir = Config::GetHomeDir() / std::to_string(user.user_id);
...
bool UserManager::RemoveUser(s32 user_id) {
    ...
    const auto user_dir = Config::GetHomeDir() / std::to_string(user_id);
    ...
    for (auto& user : m_users.user) {
        if (user.user_id == user_id) {
            if (user.user_name == new_name)
                return true; // no change, nothing to persist

            user.user_name = new_name;
            // Persist the rename. This Save() previously sat AFTER the loop, unreachable on the
            // match path (the loop returned first), so renames updated memory but never reached
            // users.json and were lost on the next Load().
            Save();
            return true;
        }
    }
    return false; // user not found -> nothing to save
}
...
Users UserManager::CreateDefaultUsers() {
    ...
    for (auto& u : default_users.user) {
        const auto user_dir = Config::GetHomeDir() / std::to_string(u.user_id);
...
std::vector<User> UserManager::GetValidUsers() const {
    ...
    const auto home_dir = Config::GetHomeDir();
