...
static void SignalHandler(int sig, siginfo_t* info, void* raw_context) {
    ...
    case SIGBUS: {
        ...
        // regularly; crashing on every one makes the game unplayable.
        {
            ...
            // GR2FORK: the null window covers both sides of address 0 - a null base with a
            // negative displacement faults just below the wrap (observed 0xffffffffffffff98 =
            // null - 0x68); no host allocation lives within 64 KiB of the wrap.
            ...
            if (null_page_pos || null_page_neg) {
                ...
                // Entity render function 0xf5b410-0xf5bdd5 called with NULL entity data would
                // loop forever under per-instruction absorption (r14 never matches rcx=0), so
                // jump to its epilogue (0xf5bdd6), which checks the canary and returns cleanly.
                ...
                if (ZYAN_SUCCESS(status)) {
                    ...
                    // GR2FORK: negative-window faults count and tag
                    // separately ("generic-neg") - see counter doc above.
                    ...
                    // GR2FORK: absorbed null reads inside std::string::assign (guest
                    // 0x1228060..0x12280d0) log the caller via [rbp+8] (ghidra address =
                    // caller - 0x800000000 + 0x107BF0), throttled separately from boot faults.
                    if (rip_lo >= 0x01228060u && rip_lo < 0x012280d0u) {
                        auto* uctx_a = static_cast<ucontext_t*>(raw_context);
                        auto& gregs_a = uctx_a->uc_mcontext.gregs;
                        const u64 rbp_a = static_cast<u64>(gregs_a[REG_RBP]);
                        const u64 src_a = static_cast<u64>(gregs_a[REG_R14]); // rsi: NULL source
                        const u64 dst_a = static_cast<u64>(gregs_a[REG_RBX]); // rdi: dst std::string
                        u64 caller_a = 0, saved_rbp_a = 0, caller2_a = 0;
                        if (rbp_a > 0x10000) {
                            saved_rbp_a = *reinterpret_cast<const u64*>(rbp_a);
                            caller_a = *reinterpret_cast<const u64*>(rbp_a + 8);
                            if (saved_rbp_a > 0x10000) {
                                caller2_a = *reinterpret_cast<const u64*>(saved_rbp_a + 8);
                            }
                        }
                        const u64 an =
                            g_absorb_assign_null.fetch_add(1, std::memory_order_relaxed) + 1;
                        if (AbsorbLogThrottle(an)) {
                            LOG_WARNING(Core,
                                        ...
                                        an, GetThreadName(), dst_a, caller_a,
                                        caller_a > 0x800000000ull
                                            ? caller_a - 0x800000000ull + 0x107BF0ull
                                            : 0,
                                        caller2_a,
                                        caller2_a > 0x800000000ull
                                            ? caller2_a - 0x800000000ull + 0x107BF0ull
                                            : 0,
                                        saved_rbp_a, src_a);
                        }
                    }
                    if (AbsorbLogThrottle(n)) {
                        ...
                        if (n <= 4) {
                            ...
                            // GR2FORK combined-binary TLS probe: the dlopen'd-core build faults
                            // here with rax=0 where the standalone exe does not. These dumps
                            // separate a wrong/unset GS base from a genuinely-null object.
...
#if defined(__linux__)
                            // Read the GS base the CPU holds on this thread: gs_base=0 or a
                            // stale value means the combined build did not set up the thread's
                            // TLS (compare GAME_MainThread against the fiber workers).
                            ...
                            // First fault only: raw guest code bytes around the faulting RIP so
                            // the instruction that produced rax=0 can be disassembled offline
                            // (gs:[disp] read = GS base suspect; [reg+disp] = null object field).
                            if (n == 1) {
                                const auto* cp =
