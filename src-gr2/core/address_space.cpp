...
void* AddressSpace::Map(VAddr virtual_addr, size_t size, u64 alignment, PAddr phys_addr,
    ...
    if (ptr != (void*)-1 && size >= 0x200000) {
        ...
        // GR2FORK PERF: the hint above is a no-op for this memfd/MAP_SHARED
        // mapping on most hosts (THP shmem_enabled defaults to `never`), so
        // the guest space runs on 4K pages - measured 714M L1-DTLB misses per
        // 30s on the assembler with zero 2M-page reloads. MADV_COLLAPSE
        // (kernel >= 6.1) collapses eligible ranges regardless of the sysfs
        // setting; failures (older kernels, non-2M-congruent offsets, memory
        // pressure) are harmless and partial collapse still applies. Cost is
        // paid at map time, not per frame. Kill switch: GR2_NOCOLLAPSE=1.
#ifndef MADV_COLLAPSE
#define MADV_COLLAPSE 25
#endif
        static const bool collapse_enabled = []() noexcept {
            const char* e = std::getenv("GR2_NOCOLLAPSE");
            return !(e && e[0] == '1');
        }();
        if (collapse_enabled) {
