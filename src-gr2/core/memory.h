...
public:
    ...
    // GR2FORK PERF: inline the dominant fast path - nearly all calls pass size well under 3_GB,
    // a no-op clamp, yet an out-of-line implementation costs a call each time (0.12% of the
    // GpuComm thread). The slow path stays in memory.cpp so this header does not pull in vma_map.
    u64 ClampRangeSizeSlow(VAddr virtual_addr, u64 size);
    inline u64 ClampRangeSize(VAddr virtual_addr, u64 size) {
        // 3_GB matches the threshold in ClampRangeSizeSlow; the two must
        // stay in sync.
