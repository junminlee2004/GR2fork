...
public:
    ...
    // GR2: expose the guest heap so instruments can malloc guest-visible buffers (e.g. to pass an
    // argument struct to a guest function called via Core::ExecuteGuest).
    AppHeapAPI GetHeapApi() const {
        return heap_api;
    }
