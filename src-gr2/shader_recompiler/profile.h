...
struct Profile {
    ...
    // GR2FORK: internal-resolution upscale factor (1.0 = disabled), baked into the clip-disabled
    // screen-space->NDC transform so passthrough draws fill a scaled render target; the per-pass
    // viewport compensates by 1/scale for native-buffer draws. Frozen at startup.
