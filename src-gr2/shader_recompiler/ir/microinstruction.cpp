...
void Inst::SetArg(size_t index, Value value) {
    ...
    // GR2FORK PERF: Phi instructions live only at control-flow merges; hint the straight-line
    // args[index] path so the dynamic phi_args[] write moves to the cold tail.
...
void Inst::ClearArgs() {
    if (op == Opcode::Phi) {
        // GR2FORK PERF: cache the size - with the SetArg/UndoUse body the compiler cannot prove
        // phi_args.size() loop-invariant and reloads it every iteration; a size_t index matches
        // .size() and avoids a zero-extension per compare.
        ...
        // GR2FORK PERF: args is std::array<Value, 6> - size is a
