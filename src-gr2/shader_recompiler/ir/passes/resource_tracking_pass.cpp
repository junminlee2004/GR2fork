...
public:
    ...
    u32 Add(const ImageResource& desc) {
        const u32 index{Add(image_resources, desc, [&desc](const auto& existing) {
            // PORT(upstream #4075): two image descriptors with the same sharp
            // but different mip-fallback bindings must be distinct entries,
            // otherwise the descriptor-set layout computed from NumBindings
            // won't match what the shader expects.
        ...
        image.is_raw_access |= desc.is_raw_access;
        return index;
    }
...
void PatchImageSharp(IR::Block& block, IR::Inst& inst, Info& info, Descriptors& descriptors,
    ...
    // PORT(upstream #4075): detect IMAGE_STORE_MIP-style ops that need the
    // descriptor-array fallback because the driver lacks native
    // image-load-store-lod support. ImageRead without is_written is OpImageFetch
    // (handled natively); only is_written + has_lod needs the fallback here.
    ...
    ImageResource image_res = {
        ...
        .is_raw_access = is_storage || is_atomic,
    ...
    if (needs_mip_storage_fallback) {
        // PORT(upstream #4075): If the mip level to IMAGE_(LOAD/STORE)_MIP is
        // a compile-time constant, bind only that single level. If the lod is
        // dynamic, bind every level of the image as an array and index at
        // runtime. The lod argument's position in the image op body depends on
        // the view type.
        const auto view_type = image.GetViewType(image_res.is_array, image_res.is_raw_access);
        ...
        const auto lod_arg = [&] -> IR::Value {
            ...
            case AmdGpu::ImageType::Cube:         // sc, tc, face, [lod]
...
// Legacy coordinate build for the Color2DArray path. Plain 2D arrays pass
// through untouched. Cube T#s routed here — cube arrays, raw-access bindings,
// VU-ineligible shapes, or the native-cube toggle off (all decided in
// AmdGpu::Image::GetViewType) — get the stock remap: AMD cube math delivers
// face-local coordinates in [1.0, 2.0]; (x - 1, y - 1) rebases them to [0, 1]
// with the face index as the array slice. This is the exact v4.4 layout for a
// 2D-array view. No toggle read here: the view type already encodes the
// decision, so the IR is correct by construction in all four
// single/array x toggle-on/off states.
...
// Native coordinate build — reached only from the ImageType::Cube case, i.e.
// when GetViewType selected a native eCube view (single sampled cube, VU-valid
// shape, toggle on).
//
// AMD cube math delivers the face-local coordinates in the range [1.0, 2.0];
// (x - 1, y - 1) are the standard cube face UVs s,t in [0,1] (the same
// mapping the legacy 2D-array path sampled, which is why face *content* was
// already correct). Reconstruct the 3D direction vector that the cube sampler
// re-projects to that same texel, so the GPU does seamless cross-face
// filtering instead of clamping at face edges.
//
// shadPS4's V_CUBESC/V_CUBETC implement the standard sc/tc convention and
// V_CUBEID the standard face order (+X,-X,+Y,-Y,+Z,-Z = 0..5), so the inverse
// of the standard cube projection applies directly. With |major| := 1:
//   sc' = 2s - 1,  tc' = 2t - 1     (= sc/|major|, tc/|major|)
//   face 0 +X: ( 1, -tc', -sc')   face 1 -X: (-1, -tc',  sc')
//   face 2 +Y: ( sc',  1,  tc')   face 3 -Y: ( sc', -1, -tc')
//   face 4 +Z: ( sc', -tc',  1)   face 5 -Z: (-sc', -tc', -1)
IR::Value FixCubeCoordsNative(IR::IREmitter& ir, const IR::Value& x, const IR::Value& y,
                              const IR::Value& face) {
    ...
    const IR::F32 pos1 = ir.Imm32(1.f);
    const IR::F32 neg1 = ir.Imm32(-1.f);

    const IR::F32 f{face};
    const IR::U1 is0 = ir.FPEqual(f, ir.Imm32(0.f));
    const IR::U1 is1 = ir.FPEqual(f, ir.Imm32(1.f));
    const IR::U1 is2 = ir.FPEqual(f, ir.Imm32(2.f));
    const IR::U1 is3 = ir.FPEqual(f, ir.Imm32(3.f));
    const IR::U1 is4 = ir.FPEqual(f, ir.Imm32(4.f));
    const IR::U1 is5 = ir.FPEqual(f, ir.Imm32(5.f));

    // rx: +X -> +1, -X -> -1, -Z -> -sc', else (+Y,-Y,+Z) -> sc'
    const IR::Value rx = ir.Select(is0, pos1, ir.Select(is1, neg1, ir.Select(is5, nsc, sc)));
    // ry: +Y -> +1, -Y -> -1, else (+X,-X,+Z,-Z) -> -tc'
    const IR::Value ry = ir.Select(is2, pos1, ir.Select(is3, neg1, ntc));
    // rz: +X -> -sc', -X -> +sc', +Y -> +tc', -Y -> -tc', +Z -> +1, -Z -> -1
    const IR::Value rz =
        ir.Select(is0, nsc,
                  ir.Select(is1, sc,
                            ir.Select(is2, tc, ir.Select(is3, ntc, ir.Select(is4, pos1, neg1)))));
    return ir.CompositeConstruct(rx, ry, rz);
}
...
void PatchImageSampleArgs(IR::Block& block, IR::Inst& inst, Info& info,
    ...
    const IR::Value offset = [&] -> IR::Value {
        ...
        case AmdGpu::ImageType::Cube:
            // Offset/ConstOffset image operands are invalid SPIR-V for Dim=Cube
            // (texel offsets cannot apply to a direction vector). The offset
            // address register was already consumed above, so the remaining
            // operand decoding stays aligned; drop the operand itself, matching
            // hardware behavior of ignoring offsets for cube samples. The legacy
            // 2D-array cube path (Color2DArray case above) keeps applying it.
            return IR::U32{};
    ...
    const IR::Value coords = [&] -> IR::Value {
        ...
        case AmdGpu::ImageType::Color2DArray: // x, y, slice
            addr_reg = addr_reg + 3;
            // Cube T#s on this path (cube arrays, raw-access bindings, ineligible
            // shapes, toggle off) take the legacy [1,2]->[0,1] face-UV remap inside
            // FixCubeCoords; plain 2D arrays pass through untouched.
            return FixCubeCoords(ir, image, get_coord(addr_reg - 3, 0), get_coord(addr_reg - 2, 1),
                                 get_addr_reg(addr_reg - 1));
        case AmdGpu::ImageType::Cube: // sc, tc, face -> reconstructed direction
            addr_reg = addr_reg + 3;
            return FixCubeCoordsNative(ir, get_coord(addr_reg - 3, 0), get_coord(addr_reg - 2, 1),
                                       get_addr_reg(addr_reg - 1));
...
void PatchImageArgs(IR::Block& block, IR::Inst& inst, Info& info) {
    ...
    // PORT(upstream #4075): when fallback binds a single mip level (ConstantIndex
    // mode), the shader must not pass a lod argument — the descriptor already
    // points at the right mip. DynamicIndex keeps the lod for runtime indexing.
    const auto lod =
