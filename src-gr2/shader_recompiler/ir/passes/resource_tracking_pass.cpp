...
public:
    ...
    u32 Add(const ImageResource& desc) {
        const u32 index{Add(image_resources, desc, [&desc](const auto& existing) {
            // PORT(upstream #4075): two image descriptors with the same sharp but different
            // mip-fallback bindings must stay distinct entries, or the descriptor-set layout
            // computed from NumBindings will not match what the shader expects.
...
void PatchImageSharp(IR::Block& block, IR::Inst& inst, Info& info, Descriptors& descriptors,
    ...
    // PORT(upstream #4075): detect IMAGE_STORE_MIP-style ops needing the descriptor-array
    // fallback when the driver lacks native image-load-store-lod. Only is_written + has_lod needs
    // it; ImageRead without is_written is OpImageFetch, which is handled natively.
    ...
    if (needs_mip_storage_fallback) {
        // PORT(upstream #4075): a compile-time-constant mip level to IMAGE_(LOAD/STORE)_MIP binds
        // only that level; a dynamic lod binds every level as an array and indexes at runtime.
        // The lod argument's position in the image op body depends on the view type.
...
// Legacy coordinate build for the Color2DArray path. Cube T#s routed here (decided in
// AmdGpu::Image::GetViewType) get the stock remap: AMD cube math delivers face-local coordinates
// in [1.0, 2.0]; (x - 1, y - 1) rebases to [0, 1] with the face index as the array slice.
...
// Native coordinate build, reached only when GetViewType selected a native eCube view. AMD cube
// math delivers face UVs in [1.0, 2.0]; rebuild the 3D direction via the inverse of the standard
// cube projection (V_CUBEID face order +X,-X,+Y,-Y,+Z,-Z) so filtering crosses face edges.
...
void PatchImageSampleArgs(IR::Block& block, IR::Inst& inst, Info& info,
    ...
    const IR::Value offset = [&] -> IR::Value {
        ...
        case AmdGpu::ImageType::Cube:
            // Offset/ConstOffset operands are invalid SPIR-V for Dim=Cube; drop them, matching
            // hardware, which ignores offsets for cube samples. The offset register was already
            // consumed above, so the remaining operand decoding stays aligned.
            return IR::U32{};
