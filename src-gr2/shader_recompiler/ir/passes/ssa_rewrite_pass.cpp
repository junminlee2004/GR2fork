...
// FIX(GR2FORK mask-spill): SSA variable family for the per-lane (U1) view of a
// thread mask spilled into a VGPR lane via V_WRITELANE_B32 and restored via
// V_READLANE_B32. Keyed by (vgpr << 6) | lane. Inherits FlagTag so
// UndefOpcode(FlagTag) -> UndefU1 applies. See Translator::V_WRITELANE_B32.
...
using Variant = std::variant<IR::ScalarReg, IR::VectorReg, GotoVariable, MaskLaneVariable,
                             ThreadBitScalar, SccFlagTag, ExecFlagTag, VccFlagTag, VccLoTag,
                             VccHiTag, M0Tag>;
