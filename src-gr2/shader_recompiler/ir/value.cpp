...
bool Value::operator==(const Value& other) const {
    ...
    // GR2FORK PERF: hoist the dominant case - most comparisons (>70% in shader compile
    // workloads) are two Type::Opaque instruction references, so a single inline compare
    // bypasses the jump table; the fall-through switch handles every other type unchanged.
