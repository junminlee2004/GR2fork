...
public:
    ...
    [[nodiscard]] size_t NumArgs() const {
        // GR2FORK PERF: Phi instructions are a tiny minority of IR (control-flow merges only);
        // an explicit if/else gives an [[unlikely]] anchor and keeps the dynamic phi_args.size()
        // load off the straight-line path every IR pass uses.
    ...
    [[nodiscard]] Value Arg(size_t index) const noexcept {
        // GR2FORK PERF: same reasoning - Phi is the rare branch in any IR pass that walks args;
        // keep the args[index] return on the straight-line fast path.
        if (op == IR::Opcode::Phi) [[unlikely]] {
