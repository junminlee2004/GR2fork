...
// GR2FORK v4.5 grass-fix latch: same kill switch as the lane-shadow half in
// data_share.cpp (the two halves of the GCN exec-mask accuracy chain flip
// together — partial combinations are known-broken). Forced on; env kill
// GR2_NOGCNMASK=1 restores the raw v4.4 V_CMPX translation: ir.SetExec(result)
// and the raw dst write, no exec-masking.
...
void Translator::V_CMP_F32(ConditionOp op, bool set_exec, const GcnInst& inst) {
    ...
    if (set_exec) {
        // V_CMPX evaluates only on active lanes; inactive lanes clear their
        // EXEC bit AND their bit in the VCC/SDST result (vcc' = exec' =
        // exec & result). Writing the raw result to VCC would leak set bits
        // for masked-off lanes into VCC consumers (e.g. s_cbranch_vccz
        // guarded regions and scalar mask arithmetic).
        // Kill switch GR2_NOGCNMASK=1: raw
        // ir.SetExec(result) and fall through to the raw dst switch below —
        // exact v4.4.
        if (Gr2GcnMaskAccuracyEnabled()) {
            ...
            case OperandField::ScalarGPR:
                ir.SetThreadBitScalarReg(IR::ScalarReg(inst.dst[1].code), masked);
                break;
            default:
                UNREACHABLE();
            }
...
void Translator::V_CMP_U32(ConditionOp op, bool is_signed, bool set_exec, const GcnInst& inst) {
    ...
    if (set_exec) {
        ...
        if (Gr2GcnMaskAccuracyEnabled()) {
            ...
            switch (inst.dst[1].field) {
            case OperandField::VccLo:
                return ir.SetVcc(masked);
            case OperandField::ScalarGPR:
                return ir.SetThreadBitScalarReg(IR::ScalarReg(inst.dst[0].code), masked);
            default:
                UNREACHABLE();
            }
        }
