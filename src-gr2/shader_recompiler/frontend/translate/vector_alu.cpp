...
// GR2FORK: same kill switch as the lane-shadow half in data_share.cpp - the two halves of the GCN
// exec-mask accuracy chain flip together (partial combinations are known-broken). GR2_NOGCNMASK=1
// restores the raw V_CMPX translation: ir.SetExec(result) and the raw dst write, no exec-masking.
...
void Translator::V_CMP_F32(ConditionOp op, bool set_exec, const GcnInst& inst) {
    ...
    if (set_exec) {
        // V_CMPX evaluates only on active lanes: inactive lanes clear their EXEC bit and their
        // bit in the VCC/SDST result (vcc' = exec' = exec & result); writing the raw result would
        // leak set bits into VCC consumers. GR2_NOGCNMASK=1 falls back to raw ir.SetExec(result).
