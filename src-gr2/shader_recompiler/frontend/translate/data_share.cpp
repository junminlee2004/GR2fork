...
// GR2FORK: latch for the GCN exec-mask accuracy chain. One key covers both the mask-spill lane
// shadows here (V_READLANE/V_WRITELANE) and the exec-masked V_CMP dst writes in vector_alu.cpp -
// the halves are only correct together. Forced on, latched on first shader translation; env kill
// GR2_NOGCNMASK=1 restores the legacy translation (no lane shadows, raw SetExec + dst writes).
...
void Translator::V_READLANE_B32(const GcnInst& inst) {
    ...
    // GR2FORK FIX: mask-spill restore. GCN compilers spill thread masks into VGPR lanes and
    // restore them via v_readlane; SetDst only defines the SGPR's U32 view, so without also
    // restoring the U1 view from the shadow saved by V_WRITELANE_B32, SSA materializes UndefU1
    // and folds exec restores / store guards to false (GR2 grass seed CS cs_0x3f0a3c48 never
    // rendered grass). An SGPR write architecturally clobbers both views, so unconditionally
    // redefining the U1 view is strictly more faithful. GR2_NOGCNMASK=1 skips the restore.
...
void Translator::V_WRITELANE_B32(const GcnInst& inst) {
    ...
    // GR2FORK FIX: mask-spill save. Shadow the per-lane U1 view of spilled thread masks so
    // V_READLANE_B32 can restore it; for plain-data sources the shadowed U1 resolves to Undef,
    // matching the default for a later mask-typed read. GR2_NOGCNMASK=1 skips the save.
    if (Gr2GcnMaskAccuracyEnabled() && lane.IsImmediate()) {
