...
struct FetchShaderData {
    u32 size = 0;
    // GR2FORK PERF: inline capacity 16 covers the common GR2 vertex layout (~10-12 attributes),
    // so the frequent FetchShaderData copies avoid heap malloc/free; >16 attributes spills to
    // heap like std::vector. Hash/equality/serialization are element-wise, so behavior matches.
