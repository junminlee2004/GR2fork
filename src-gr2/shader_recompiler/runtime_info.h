...
struct fmt::formatter<Shader::Stage> {
    ...
    auto format(const Shader::Stage stage, format_context& ctx) const {
        // GR2FORK FIX: bounds-check the names lookup - a Stage value outside [0..6] indexes out of
        // bounds and the garbage pointer can be null, which fmt rejects by terminating the process
        // (movie playback init, ep03_00200m). Print the raw index so the source can be traced.
