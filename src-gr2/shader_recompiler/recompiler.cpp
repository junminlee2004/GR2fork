...
namespace {
// GR2FORK internal-resolution scaling: per-consumer FragCoord correction. Integer-convert
// consumers (texelFetch) index the scaled buffer and keep the raw value; every other consumer gets
// FragCoord * (1/scale), fixing normalized UVs. x/y only; no-op at 1.0; scale is a cache-key input.
...
IR::Program TranslateProgram(const std::span<const u32>& code, Pools& pools, Info& info,
    ...
    // GR2FORK internal-resolution scaling: correct FragCoord per-consumer (native for normalized
    // screen UVs, scaled left intact for integer texelFetch). No-op at factor 1.0; placed before
    // the final cleanup so the inserted multiply and redirected uses are folded/DCE'd.
