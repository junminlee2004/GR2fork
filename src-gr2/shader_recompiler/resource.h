...
struct ImageResource {
    ...
    bool is_array{};
    bool is_written{};
    // True when any instruction accesses this binding with raw integer texel
    // coordinates (IMAGE_LOAD / IMAGE_STORE / atomics) rather than a sampler.
    // Merged across instructions in Descriptors::Add. A cube T# with any raw
    // access stays on the legacy 2D-array path (see AmdGpu::Image::GetViewType):
    // OpImageFetch is invalid for Dim=Cube and Metal cannot write cube textures.
    bool is_raw_access{};
    bool is_r128{};
