...
Id EmitImageGradient(EmitContext& ctx, IR::Inst* inst, u32 handle, Id coords, Id derivatives_dx,
    ...
    if (texture.view_type == AmdGpu::ImageType::Cube) {
        // FIX(GR2FORK): a native cube sample takes a 3D direction and 3-component
        // gradients, but GCN supplies 2-component face-space derivatives that we
        // do not convert. Emitting those would be invalid for a Dim=Cube image,
        // so drop them and sample at explicit LOD 0 (valid in any shader stage,
        // unlike implicit-LOD). Gradient sampling of a cube is rare outside mip
        // selection; the visible cost is limited to that case.
