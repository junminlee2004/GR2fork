...
Id EmitImageGradient(EmitContext& ctx, IR::Inst* inst, u32 handle, Id coords, Id derivatives_dx,
    ...
    const Id sampler = ctx.OpLoad(ctx.sampler_type, ctx.samplers[handle >> 16]);
    const Id sampled_image = ctx.OpSampledImage(texture.sampled_type, image, sampler);
    if (texture.view_type == AmdGpu::ImageType::Cube) {
        // GR2FORK FIX: GCN supplies 2-component face-space derivatives that are not converted
        // to the 3-component gradients a Dim=Cube image requires, so drop them and sample at
        // explicit LOD 0 (valid in any shader stage, unlike implicit-LOD).
        ImageOperands cube_operands;
        cube_operands.Add(spv::ImageOperandsMask::Lod, ctx.f32_zero_value);
        const Id sample = ctx.OpImageSampleExplicitLod(
            result_type, sampled_image, coords, cube_operands.mask, cube_operands.operands);
        return texture.is_integer ? ctx.OpBitcast(ctx.F32[4], sample) : sample;
    }
    ImageOperands operands;
...
Id EmitImageRead(EmitContext& ctx, IR::Inst* inst, u32 handle, Id coords, Id lod, Id ms) {
    ...
    if (!texture.is_storage) {
        ...
        // PORT(upstream #4075): mip-fallback for storage-image reads. IMAGE_LOAD_MIP translates
        // to OpImageFetch (non-storage branch), so this branch is currently unreachable; the
        // #else path is retained for when a real case is discovered.
