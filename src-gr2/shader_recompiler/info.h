...
struct Info : InfoPersistent {
    ...
    void PushUd(Backend::Bindings& bnd, PushData& push) const {
        ...
        // GR2FORK PERF: `mask &= mask - 1` compiles to single-cycle BLSR on BMI1 targets, while
        // `mask &= ~(1U << index)` may emit SHL+NOT+AND (~3 cycles). Saves ~2 cycles per set bit
        // (8-16 per call for 4-8 user_data registers); this fires per-stage per-draw.
        while (mask) {
