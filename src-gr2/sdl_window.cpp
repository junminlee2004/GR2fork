...
void SDLInputEngine::Init() {
    ...
    // Primary pad (drives imgui nav + LED/rumble focus): GUI-selected, else the configured
    // default, else the first enumerated pad. All pads are opened regardless so any connected
    // controller can drive the game.
...
WindowSDL::WindowSDL(s32 width_, s32 height_, Input::GameController* controller_,
    ...
    // GR2FORK FIX: gamescope/some Wayland compositors fire the construct-time fullscreen
    // RESIZED event late or never; ImGui then reads stale ctor dimensions and the Frame blit
    // downscales 4K to 1080p. SDL_SyncWindow (up to ~100ms) settles the WM before read-back.
    ...
    // GR2FORK FIX: Steam's runtime injects SDL_*_IGNORE_DEVICES so games only see the Steam
    // Virtual Gamepad (no gyro/accelerometer); clearing the list re-exposes the native pad and
    // its sensors. SDL_HINT_OVERRIDE beats the env-set hint; must precede SDL_INIT_GAMEPAD init.
