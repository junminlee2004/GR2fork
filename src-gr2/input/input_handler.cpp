...
// ---------------------------------------------------------------------------
// gr2fork: hold-combo touchpad swipes.
//
// While the configured hold input is held, the four combo direction inputs
// play back touchpad swipes (via TriggerButtonSwipe) INSTEAD of their normal
// mappings. Configured through the input config file:
//
//   touchpad_swipe_combo_enabled = true|false        (default false)
//   touchpad_swipe_combo_hold = l3                   (single input token)
...
//   touchpad_swipe_combo_hold_passthrough = true|false (default false)
//
// Tokens accept controller buttons, keyboard/mouse keys, and the trigger
// axes (l2/r2); stick axes and output-only names are rejected at parse time.
// Missing keys fall back to the defaults above, so a bare
// "touchpad_swipe_combo_enabled = true" gives hold-L3 + face buttons.
// ---------------------------------------------------------------------------
struct TouchpadSwipeComboConfig {
    bool enabled = false;
    // When true, the hold input keeps driving its normal mappings while held.
    // When false (default), the hold input is a dedicated modifier: its normal
    // mappings are suppressed for as long as it is held.
    bool hold_passthrough = false;
    InputID hold = InputID(InputType::Controller, SDL_GAMEPAD_BUTTON_LEFT_STICK);
    // Indexed by ButtonSwipeDirection (BUTTON_SWIPE_UP/DOWN/LEFT/RIGHT).
    InputID dir_keys[4] = {
...
// Parses a single-input token for a touchpad_swipe_combo_* slot.
// "unmapped", empty, or invalid tokens return an invalid InputID (slot off).
...
void ParseInputConfig(const std::string game_id = "") {
    ...
    connections.clear();
    // gr2fork: reset the hold-combo touchpad swipe layer so a reload (F8/IPC)
    // always starts from the documented defaults before the file is parsed.
    swipe_combo = TouchpadSwipeComboConfig{};
    float mouse_deadzone_offset = 0.5;
    float mouse_speed = 1;
    float mouse_speed_offset = 0.125;

    leftjoystick_deadzone = {5, 127};
    ...
    auto ProcessLine = [&]() -> void {
        ...
        if (output_string == "mouse_to_joystick") {
            ...
            // gr2fork: hold-combo touchpad swipes (see TouchpadSwipeComboConfig above).
    ...
    // gr2fork: sanity-check the hold-combo touchpad swipe layer now that both
    // the global and the per-game file have been parsed.
    ...
    if (swipe_combo.enabled) {
        if (!swipe_combo.hold.IsValid()) {
            LOG_WARNING(Input, "touchpad_swipe_combo_enabled is true but the hold input is "
                               "unmapped; the combo layer is inert");
        } else {
            LOG_INFO(Input,
                     "Touchpad swipe combo enabled: hold {} + up {}, down {}, left {}, right {} "
                     "(hold passthrough: {})",
                     swipe_combo.hold.ToString(), swipe_combo.dir_keys[BUTTON_SWIPE_UP].ToString(),
                     swipe_combo.dir_keys[BUTTON_SWIPE_DOWN].ToString(),
                     swipe_combo.dir_keys[BUTTON_SWIPE_LEFT].ToString(),
                     swipe_combo.dir_keys[BUTTON_SWIPE_RIGHT].ToString(),
                     swipe_combo.hold_passthrough);
        }
    }
...
// gr2fork: pre-pass for the hold-combo touchpad swipe layer.
// Runs before the normal connections loop so that, while the hold input is
// held, fresh presses of the four combo direction inputs play back touchpad
// swipes and have their normal mappings suppressed (consumed) until release.
static void ProcessTouchpadSwipeCombo() {
    if (!swipe_combo.enabled || !swipe_combo.hold.IsValid()) {
        return;
    }

    // Finds the pressed_keys entry for a given input, if any.
    auto FindPressed = [](const InputID& id) {
        return std::find_if(
            pressed_keys.begin(), pressed_keys.end(),
            [&id](const std::pair<InputEvent, bool>& entry) { return entry.first.input == id; });
    };
    // Whether a pressed_keys entry counts as "held". Axis entries persist in
    // the list with a live axis_value, so triggers (l2/r2) are gated on the
    ...
    if (hold_down && !swipe_combo.hold_passthrough) {
        // Dedicated modifier (default): consume the hold input so its normal
        // mappings stay off for as long as it is held.
        hold_it->second = true;
    }
    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        if (dir_down && !swipe_combo.dir_was_down[dir] && hold_down) {
            // Fresh press while the hold is down: this press belongs to the
            // combo. Play back the swipe and latch ownership until release.
            TriggerButtonSwipe(ControllerOutput::GetController(), dir);
            swipe_combo.dir_swipe_owned[dir] = true;
            LOG_DEBUG(Input, "Touchpad swipe combo fired: {}", swipe_combo_dir_names[dir]);
        }
        if (!dir_down) {
            // Physical release ends the combo's claim on this input.
            swipe_combo.dir_swipe_owned[dir] = false;
        }
        if (swipe_combo.dir_swipe_owned[dir] && dir_it != pressed_keys.end()) {
            // Suppress the normal mapping for as long as the combo owns this
            // press, even if the hold input was released first.
            dir_it->second = true;
        }
...
void ActivateOutputsFromInputs() {
    ...
    // gr2fork: hold-combo touchpad swipes run as a pre-pass so they can
    // consume their inputs before the normal bindings see them.
