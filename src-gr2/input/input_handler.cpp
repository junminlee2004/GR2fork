...
// gr2fork: hold-combo touchpad swipes - while the configured hold input is held, the four combo
// direction inputs play back touchpad swipes instead of their normal mappings (default hold-L3 +
// face buttons). Keys: touchpad_swipe_combo_{enabled,hold,up,down,left,right,hold_passthrough}.
...
void ControllerOutput::FinalizeUpdate() {
    ...
    if (button != SDL_GAMEPAD_BUTTON_INVALID) {
        ...
        case SDL_GAMEPAD_BUTTON_TOUCHPAD_DOWN:
            ...
            controller->CheckButton(0, SDLGamepadToOrbisButton(button), new_button_state);
            break;
        // Synthetic touchpad-swipe outputs are rising-edge-only and fire a timed center ->
        // direction -> release sequence; the SDL_AddTimer chain in input_mouse.cpp owns playback.
        case SDL_GAMEPAD_BUTTON_TOUCHPAD_SWIPE_UP:
            if (new_button_state) {
...
// gr2fork: pre-pass for the hold-combo touchpad swipe layer. Runs before the normal connections
// loop so that, while the hold input is held, fresh presses of the combo direction inputs play
// back touchpad swipes and have their normal mappings suppressed until release.
