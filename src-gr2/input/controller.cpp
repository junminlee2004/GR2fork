...
void GameController::CalculateOrientation(Libraries::Pad::OrbisFVector3& acceleration,
    ...
    // GR2FORK: clamp the integration step. deltaTime is raw wall-clock since the
    // previous pad poll; m_last_update defaults to the steady_clock epoch (an
    // enormous dt on the very first poll) and a frame stall (e.g. ISS's heavy
    // shader-compile spikes) produces a large dt. This is a first-order Euler
    // step (q += qDot * dt); when dt is large, qDot*dt dwarfs q, so after
    // normalization the attitude snaps to the instantaneous angular-velocity
    // direction — the orientation "flips" and a motion-aimed camera points the
    // wrong way (the inFAMOUS Second Son "camera points above Delsin" softlock).
    // Cap dt so a stale/abnormal gap can't corrupt the attitude; also guard
    // NaN/non-positive. Normal frame pacing (<=50 ms) is unaffected.
