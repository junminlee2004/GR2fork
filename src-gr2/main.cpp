...
int main(int argc, char* argv[]) {
    ...
    Config::load(user_dir / "config.json");

    // GR2FORK: load (and on first run create) the PS4 user accounts -> users.json,
    // matching the prerelease layout, then log in the default user so the user
    // service reports one logged-in user. Done here rather than via controller
    // connect events to leave gr2fork's custom controller input untouched.
    UserSettings.Load();
    if (User* default_user = UserManagement.GetUserByPlayerIndex(1)) {
        UserManagement.LoginUser(default_user, 1);
    }

    // GR2FORK: load (and on first run create) the cryptographic keys -> keys.json,
    // matching the prerelease. On first run with no keys.json, the existing trophy
    // key from config.json (GR2Fork.trophyKey) is migrated in, so trophies keep
    // working and the two builds share one keys.json.
    ...
    std::unordered_map<std::string, std::function<void(int&)>> arg_map = {
        ...
        {"--add-game-folder",
         [&](int& i) {
             ...
             Config::save(Common::FS::GetUserPath(Common::FS::PathType::UserDir) / "config.json");
