...
int main(int argc, char* argv[]) {
    ...
    // GR2FORK: load (or create on first run) the PS4 user accounts (users.json) matching the
    // prerelease layout, then log in the default user. Done here rather than via controller
    ...
    // GR2FORK: load (or create on first run) the cryptographic keys (keys.json) matching the
    // prerelease. With no keys.json, the trophy key from config.json (GR2Fork.trophyKey) is
    // migrated in so trophies keep working and the two builds share one keys.json.
