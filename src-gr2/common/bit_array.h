...
public:
    ...
    // GR2FORK PERF: all-set mirror of AnyInRange ("is every bit in [start, end) set?") for the
    // written-bind no-op peek in MemoryTracker::ForEachUploadRange. Degenerate/empty ranges
    // return false - conservative for that caller (no skip); the locked path handles them.
    [[nodiscard]] bool AllInRange(size_t start, size_t end) const {
        if (start >= end || start >= N) {
            return false;
        }
        if (end > N) {
        ...
        if (first_word == last_word) {
            const u64 m = start_mask & end_mask;
            return (data[first_word] & m) == m;
        }
