...
// PERF(GR2FORK): number of physical cores on the host, counted by the SAME
// /sys (Linux) / GetLogicalProcessorInformationEx (Windows) enumeration that
// backs the GpuComm/assembler core-reservation logic below. Sharing one
// enumerator keeps the boot-time "is this a 4-physical-core host?" check (which
// gates the legacy monolithic GpuComm path; see Config::isLegacyMonolithicGpuComm
// and emulator.cpp) consistent with how the reservation code counts cores.
//
// SMT / hyperthreads are NOT counted: a 4c/8t host (e.g. Steam Deck Aerith) and
// a 4c/4t host both return 4. Returns 0 when the count can't be determined —
// enumeration failed, or an unsupported platform (macOS / *BSD, where the
// affinity code also no-ops). Callers should treat 0 as "unknown" and not force
// topology-dependent behavior on it.
...
// GR2FORK: cores that secondary worker threads (async pipeline-compile workers
// and the cache-storage compression I/O worker) must avoid, so they never
// contend with GpuComm or the GpuAssembler on those threads' own cores. Normally
// both hot-core pins OR'd together. Returns 0 in non-exclusive mode
// (Config::isGpuCoresNonExclusive), where the pins stay but the cores are shared,
// so those workers may use them freely. Suitable for ~mask into an affinity set.
...
// GR2FORK: the mask ExcludeReservedCoresFromAllOtherThreads would strip from
// other threads, with the SAME gating — returns (GpuComm pin | assembler pin)
// only when the exclusion walk would actually run (exclusive mode AND a
// dedicated free physical core exists), else 0. Use this to apply the walk's
// effect to the CALLING thread immediately instead of waiting for the next
// walk/rewalk pass: SetCurrentThreadAffinityMask(~GetExclusionStripMask())
// (the setter clips to real host CPUs) yields exactly the steady-state mask
// the walk would eventually impose — host_cpus & ~reserved. Used by the
// guest-thread trampoline (RunThread in pthread.cpp) so threads are fenced
// at birth (critical on Windows, where CreateThread doesn't inherit affinity
// and the periodic re-walk otherwise leaves an up-to-5s window per spawn),
// and by the hang watchdog. Returns 0 in non-exclusive mode, on hosts with
// no reserved core, and on fallback-pool topologies (4c/8t Steam Deck etc.)
// — the same cases where the walk itself no-ops, so callers narrow exactly
// when the walk would.
//
// KEEP IN SYNC with the gate sequence at the top of
