...
// GR2FORK: serialize std::filesystem::path as a UTF-8 json string, byte-identical
// to the prerelease's serializer (core/emulator_settings.cpp). Required so paths
// written by gr2fork (home_dir, sys_modules_dir, addon_install_dir, install_dirs)
// are read back correctly by the prerelease build and vice-versa.
namespace nlohmann {
template <>
...
public:
    ...
    // GR2FORK: json mirrors of setFromToml/setTomlValue. setFromJson reads a single
    // key out of an already-parsed section object; a missing key or a type mismatch
    // leaves the current value untouched (base_value kept, or game_specific_value
    // left empty), matching the toml::get_optional semantics above.
...
// GR2FORK GpuAsse: spec-fingerprint -> permutation LRU. Resolves a shader
// permutation without constructing StageSpecialization when only resource
// ADDRESSES changed between draws (GR2's per-draw UBO-pointer SGPR churn,
// which pipelineUdHashLruEnable cannot catch because ud_hash includes the raw
// user_data bytes). Address-independent key, validated collision-free; ~97%
// reclaim on GR2. Default ON. Set false to disable if visual artifacts appear.
static ConfigEntry<bool> pipelineSpecFpLruEnable(true);
// GR2FORK GpuAsse: skip RefreshGraphicsKey's context-register rebuild on draws
// whose only change was a user_data-only SH re-emit (no context-key reg changed).
// Modules are still re-resolved and the full key memcmp guards reuse; ~99.7% skip
// on GR2. Default ON. Set false to disable if visual artifacts appear.
...
// ===========================================================================
// GR2FORK v4.5 GRAPHICS-FIX TOGGLE ([GR2Fork] section; launcher "Debug" tab).
//
// As of 2026-06-15 the v4.5 fps optimizations are NO LONGER config-controlled:
// they are forced on unconditionally and can only be killed individually via
// their per-site GR2_NO* env switches (instrument-don't-guess bisection still
// works; production configs can no longer disable a shipped optimization).
// Graphical regressions on GCN-era parts (Polaris RX 400/500) were confirmed
// to come PURELY from the skybox cubemap fix, so that fix is the one and only
// GR2 change that keeps a persisted config key — a user who hits cube-seam
// corruption can turn it off without a custom build. Its env kill
// (GR2_NOCUBEVIEW=1) still overrides on top of the config layer (house A/B
// rule).
//
// Everything else from the old "Optimizations" tab — gr2Opt* fps levers, the
// gr2Fix* GCN-mask/null-sharp fixes, and the gcnCompatMode master — is gone
// from config entirely (env-only now). The legacy [Vulkan] keys for the old
// assembler gates remain deliberately IGNORED (stale-config history,
// 2026-06-10 incident).
// ===========================================================================
...
// once at startup from emulator.cpp's authoritative SKU detection (the same
// boolean fed to setGameSpecificCacheToggles). Read by GPU-side code that needs
// to size structures differently for GRR (e.g. the GpuComm snapshot pool / the
// bundle-assembler intent queue run a shallower ring on Remastered). Set on the
// startup thread before any GPU object is constructed, so a plain bool read is
// safe by the time the renderer's worker threads exist.
static bool g_is_gr_remastered = false;
static bool g_is_infamous_second_son = false;

// GR2FORK: per-session "force the legacy monolithic GpuComm path?" flag. NOT
// persisted (not a ConfigEntry) — it is a runtime decision computed once at
// startup in emulator.cpp from (host physical-core count == 4 && this title is
// Gravity Rush Remastered). When true, BundleAssembler runs as a synchronous
// inline dispatcher with no GpuAssembler jthread, and DecideReservedCores skips
// the assembler core reservation so the guest keeps that physical core. Set on
// the startup thread before any GPU object (and therefore any BundleAssembler)
// is constructed, so a plain bool read is safe by the time the renderer's
// worker threads exist.
static bool g_legacy_monolithic_gpucomm = false;

// GR2FORK: when true, GpuComm and the GpuAssembler keep their core pins but those
// cores are NOT isolated from other threads (guest, async pipeline-compile
// workers, the cache-storage compression I/O worker) — they all see the full host
// CPU set. Auto-forced on a 4-physical-core host running Gravity Rush Remastered
// (see emulator.cpp); addresses the compile-burst core starvation the two-core
// reservation otherwise causes there. Mutually exclusive with
// g_legacy_monolithic_gpucomm. Read by Common::ExcludeReservedCoresFromAllOther-
// Threads and Common::GetHotCoreFenceMask. Set once at startup before any GPU /
// worker thread exists, so a plain bool read is safe. Not persisted to TOML.
...
std::filesystem::path GetSaveDataPath() {
    // GR2FORK: compat shim. Savedata now lives under the PS4 home dir, matching the
    // prerelease layout (home_dir/<uid>/savedata/<serial>). The SaveInstance path
    // builders below append "<uid>/savedata"; callers of this getter that want the
    // savedata root should prefer GetHomeDir().
    return GetHomeDir();
}
...
u32 getInternalScreenWidth() {
    // REVERTED to v3.0 behavior (returns height -> square display) to isolate the
    // loading-hang regression. Do NOT re-fix to internalScreenWidth.get() without
    // re-testing the area-transition / AvPlayer-loop hang.
...
// ---------------------------------------------------------------------------
// GR2FORK FORCED-ON PERF GATES (2026-06-10 incident) + v4.5 CONFIG REVIVAL.
//
// These six assembler-perf skips are project-baseline ON, but each was
// silently disableable by a persisted TOML key: a config file that ever
// carried `false` (an old A/B session, a launcher rewrite, an old-binary
// save) re-persists it on every save, and code defaults only govern keys
// ABSENT from the file. Found in production: shDynamicDirtySkip=false and
// descSetBindingSkipCache=false in the per-title GR2 config. D3' off makes
// every per-draw user_data SH re-emit raise gfx_key_ctx_dirty_, which kills
// the Level-2.5 key skip, the STAGE-MEMO, and P1 in one chain (ctx_stable=0
// across 11.5M GetProgram calls, shad_log 2026-06-10).
//
// Decision (Jun, 2026-06-10): force these ON in code — immune to TOML
// contents, load order, and external config rewrites. The LEGACY [Vulkan]
// ConfigEntry/TOML machinery below is left intact but its value is STILL
// IGNORED here. v4.5 update (2026-06-15): the four assembler gates that were
// briefly config-revived via [GPU] gr2Opt* keys are now forced on again with
// NO config key at all — only their per-site GR2_NO* env kills remain. The
// other two were never config-controllable:
//     beginRenderingCache      config-disable BANNED (measured fps loss);
//                              env GR2_NOBRCACHE only
//     descSetBindingSkipCache  machinery removed by BSC-STRIP; getter is
//                              unreferenced and the gate is inert
// A/B per the house rule via env kill switches, each read once per process:
//     GR2_NOBRCACHE=1     beginRenderingCache  (disabling = measured fps loss)
//     GR2_NOSHDYNSKIP=1   shDynamicDirtySkip (D3')
//     GR2_NOUDHASHLRU=1   pipelineUdHashLru
//     GR2_NOSPECFPLRU=1   pipelineSpecFpLru (~97% spec reclaim on GR2)
//     GR2_NOKEYCTXSKIP=1  pipelineGfxKeyCtxSkip (Level 2.5)
//     GR2_NODESCSKIP=1    descSetBindingSkipCache (dead post-BSC-STRIP)
// All are echoed at boot in emulator.cpp so the active state — including
// any env kill switch — is unambiguous in every log.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// GR2FORK (v4.5): the sole surviving config-controlled GR2 toggle. The skybox
// cubemap fix is the one v4.5 change confirmed to cause GCN-era graphical
// regressions, so it keeps a persisted key ([GR2Fork] gr2FixNativeCubeViews,
// default true); its GR2_NOCUBEVIEW=1 env kill latches at the consuming site
// (resource.h). Every other v4.5 opt/fix is forced on with env-only kills and
// no longer has a config getter. First-consult only after Config::load.
// ---------------------------------------------------------------------------
bool gr2FixNativeCubeViews() {
    return gr2FixNativeCubeViewsEnable.get();
}

// gr2fork: setter for the sole surviving GR2 config toggle (cubemap fix). Used
...
// per-game config has loaded, passing the per-title force-ON decisions (the RT
// cache shim is needed by both GRR and inFAMOUS Second Son; the VB cache shim
// is GRR-only). is_game_specific=true marks these as per-title overrides.
