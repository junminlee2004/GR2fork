...
// ---------------------------------------------------------------------------
// FIX(GR2FORK): host RAM sampling. Rationale: a silent process death with
// ZERO crash-handler output is exactly what memory exhaustion produces —
// the Linux OOM killer SIGKILLs (uncatchable by any user-mode handler) and
// Windows commit exhaustion turns allocation failures into __fastfail
// (bypasses VEH/SEH). Neither leaves a Critical line. Logging host RAM on
// every watchdog tick makes the correlation readable straight off a
// reporter log: if the last tick lines show sysFree collapsing toward zero
// before the log cuts off, the death was memory exhaustion, not a code
// bug. (shad_log_92 could rule out VRAM OOM because vram was on the tick
// line; host RAM was the blind spot this closes.) Sampled on the watchdog
// thread at 1 Hz — zero cost on any hot path, always-on like the rest of
// the tick line.
// ---------------------------------------------------------------------------
...
static void WatchdogLoop() {
    ...
    // PERF(GR2FORK): fence the watchdog off the GpuComm/GpuAssembler reserved
    // cores at entry. The exclusion walk at GPU init already narrows this
    // thread (it exists before Liverpool::Process runs the walk), so this is
    // belt-and-suspenders: it removes the dependence on walk timing and keeps
    // the watchdog off those cores even before GPU init. Ordering-safe: the
    // GR2_GPUCOMM_FIX config flags are set at emulator.cpp:465-466, before
    // HangWatchdog::Start at :728, and GetExclusionStripMask reads them live
    // (DecideReservedCores is not result-cached). Gated identically to the
    // walk, so this no-ops exactly where the walk would (non-exclusive mode,
    // fallback-pool topologies like 4c/8t Steam Deck, no reserved core).
    ...
    while (s.running.load(std::memory_order_acquire)) {
        ...
        // FIX(GR2FORK): host-RAM exhaustion warnings, edge-triggered with 25%
        // re-arm hysteresis so a system hovering at the threshold doesn't spam
        // at 1 Hz. These are the lines that explain a silent death: an OOM
        // kill / commit failure right after one of these is memory
        // exhaustion, not a code crash.
        if (ram.sys_total > 0) {
