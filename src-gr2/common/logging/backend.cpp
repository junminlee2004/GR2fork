...
public:
    ...
    // Crash-handler entry point - same as Stop() but safe from the backend thread itself: detects
    // self-invocation and skips the join to avoid deadlock. The caller still sets g_force_sync so
    // subsequent producer threads take the inline path.
    ...
    // Non-destructive flush: push each backend's buffer to the OS without touching the async
    // queue or the backend thread. In async mode this only flushes bytes the backend thread has
    // already written; queued entries are drained by the thread or by Stop() on shutdown.
    void FlushBackends() {
    ...
    void PushEntry(Class log_class, Level log_level, const char* filename, unsigned int line_num,
        ...
        if (Config::getLogType() == "async" &&
            ...
            // Sync path: configured, or the crash handler flipped g_force_sync. Writing inline
            // with fflush before returning is what makes post-crash LOG_CRITICAL output actually
            // appear in shad_log.txt.
...
void Flush() {
    // Non-destructive, repeatable flush (see backend.h). No-op if logging was
    // never initialized or has been torn down.
    if (!Impl::IsActive()) {
        return;
    }
...
void StopAsyncAndForceSync() {
    ...
    // Drain the queue and stop the backend thread; afterwards PushEntry's inline branch writes
    // straight to the file backend with fflush, so crash-handler LOG_CRITICAL reaches
    // shad_log.txt before the process dies.
