...
public:
    ...
    // Non-destructive flush: push each backend's buffer to the OS without
    // touching the async queue or the backend thread. Used by Common::Log::
    // Flush(). In async mode this only flushes bytes the backend thread has
    // already written (queued-but-unwritten entries are drained by the thread,
    // or by Stop()/StopAsyncAndForceSync() on shutdown).
    void FlushBackends() {
