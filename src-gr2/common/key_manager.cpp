...
bool KeyManager::TransferTrophyKey() {
    // GR2FORK: migrate the trophy key from gr2fork's config.json (GR2Fork.trophyKey, via
    // Config::getTrophyKey()) into keys.json so existing installs keep working and keys.json
    // exists before the launcher's key dialog is used; gr2fork has no config.toml [Keys] section.
    try {
