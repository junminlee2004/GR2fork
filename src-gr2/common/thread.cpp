...
ReservedCoreDecision DecideReservedCores() {
    ...
    // GR2FORK monolithic: when the legacy monolithic GpuComm path is forced
    // (4-physical-core host running Gravity Rush Remastered — see
    // Config::isLegacyMonolithicGpuComm and emulator.cpp), there is NO
    // GpuAssembler jthread to pin, so we must NOT steal a second physical core
    // for it. Skipping this block leaves assembler_mask / assembler_is_dedicated
    // / assembler_steal_from_guest at their zero defaults, which makes
    // GetGuestExcludedCoreMask strip only GpuComm's core from guest threads and
    // ExcludeReservedCoresFromAllOtherThreads protect only GpuComm's core. On a
    // 4c/8t host this returns the would-be assembler core to the guest (3 phys
    // cores instead of 2). GpuComm still gets its dedicated core — it now does
    // all the parse+assemble+record work on it.
...
u64 GetHotCoreFenceMask() {
    // Cores that secondary worker threads (async pipeline-compile workers, the
    // cache-storage compression I/O worker) must avoid so they never land on the
    // GpuComm core or the GpuAssembler core while those threads are busy. Normally
    // that is both hot-core pins OR'd together.
    //
    // GR2FORK non-exclusive mode (4-physical-core host running GRR — see
    // Config::isGpuCoresNonExclusive and emulator.cpp): the pins stay but the
    // cores are deliberately NOT isolated. Returning 0 disables both worker
    // fences, so the scheduler may place a compile on the GpuComm or GpuAssembler
    // core precisely when those threads are idle during a compile burst (GpuComm
    // spinning in the snapshot-pool yield-loop, the assembler parked in a
    // pipeline-compile wait). Reclaiming those two otherwise-wasted cores is what
    // frees the burst on a core-starved host.
...
u64 GetExclusionStripMask() {
    // GR2FORK: same gate sequence as ExcludeReservedCoresFromAllOtherThreads
    // directly below — KEEP IN SYNC. Returns the strip mask only when the
    // walk would actually run; 0 in every case the walk no-ops (non-exclusive
    // mode, no reserved core, fallback-pool topology). Callers apply it to
    // themselves via SetCurrentThreadAffinityMask(~mask) to get the walk's
    // steady-state result (host_cpus & ~reserved, via the setter's clip)
    // immediately at thread start instead of up to 5 s later via the re-walk.
    if (Config::isGpuCoresNonExclusive()) {
        return 0;
    }
...
unsigned ExcludeReservedCoresFromAllOtherThreads() {
    // GR2FORK non-exclusive mode: keep the GpuComm/GpuAssembler self-pins but do
    // NOT strip those cores from any other thread. This is the 4-physical-core
    // GRR fix (Config::isGpuCoresNonExclusive, set in emulator.cpp). With only two
    // physical cores left after a two-core reservation, isolating them starves
    // GRR's compile bursts; skipping the strip walk lets every other thread keep
    // the full host CPU set. The two self-pins (applied by GpuComm and the
    // assembler themselves, via Get*CoreMask) are unaffected. This also
    // neutralizes the periodic re-walk, which calls back into this function.
    //
    // NOTE: the gate sequence here (non-exclusive -> mask==0 ->
    // !is_dedicated_physical_core) is mirrored by GetExclusionStripMask
...
void StartPeriodicAffinityRewalk() {
    ...
    std::call_once(started, []() {
        std::thread([]() {
            ...
            // FIX(GR2FORK): get this thread itself off the hot cores. It is
            // spawned by the GpuComm thread AFTER GpuComm self-pinned to
            // GetReservedCoreMask(), so on Linux pthread_create inheritance
            // births it pinned to exactly GpuComm's isolated cores (e.g.
            // {7,15} on Z1 Extreme) — and the walk below skips self_tid, so
            // it would otherwise live on GpuComm's core for the whole
            // process lifetime. On Windows it instead arrives with the full
            // process mask (CreateThread doesn't inherit) and, being its own
            // walker, would never get narrowed either. Apply the complement
            // of the RAW pins (deliberately not GetExclusionStripMask): even
            // in non-exclusive / fallback-pool modes, where other threads
            // are allowed on those cores, there is no reason for THIS thread
            // to sit on them — and on Linux fallback-pool hosts the
            // inherited mask would otherwise trap it on GpuComm's only
            // logical CPU. The setter clips to real host CPUs, so ~hot
            // resolves to host_cpus & ~hot.
