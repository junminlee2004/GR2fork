...
static auto UserPaths = [] {
    ...
    // UserManager::CreateDefaultUsers(): four user accounts 1000..1003, each with
    // savedata/trophy/inputs subfolders. gr2fork stays single-user and only uses
    // account 1000 (ORBIS_USER_SERVICE_USER_ID_DEFAULT in userservice.cpp), and
    // only writes into savedata/, but the full set is created empty for parity so
    // both builds share one home tree. "inputs" is the per-account input folder
    // (matching the prerelease) and is unrelated to gr2fork's per-game custom
    // input config. Every create is guarded (create_directories + error_code) and
    // never overwrites or clears an existing folder.
    {
    ...
    // MIGRATION(GR2FORK): the savedata layout moved from the legacy
    // user/savedata/<uid>/<serial> into the PS4 home layout
    // user/home/<uid>/savedata/<serial> (matching the prerelease build), so the
    // two builds share one save tree. Move existing default-location saves so
    // they are still found. Only the default location is handled — a custom save
    // path set in the config is not auto-migrated — and every destination is
    // guarded so nothing is ever overwritten. Runs after HomeDir is created.
    //
    // The single-user default id also moved from 1 to 1000 to match the
    // prerelease default user, so the legacy "1" account is folded into "1000"
    // here: both the pre-home-layout user/savedata/1 and an older gr2fork
    // user/home/1/savedata are consolidated into user/home/1000/savedata.
    {
        ...
        // Pass 2: consolidate an older gr2fork user/home/1/savedata into
        // user/home/1000/savedata, then drop the now-empty "1" account if and
        // only if it is fully empty (remove() never deletes a non-empty dir, so
        // any unmerged save keeps the old account around for manual reconcile).
