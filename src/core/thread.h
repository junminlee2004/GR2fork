...
//
// Y-3 (PERF/GR2FORK): on heterogeneous (hybrid) hosts — Intel Alder/Raptor/
// Meteor Lake (P + E cores), AMD Strix Point (Zen 5 + Zen 5c), ARM big.LITTLE
// — picking is biased toward P-class (large / high-capacity) cores. The
// host's classification is read from /sys/devices/system/cpu/cpuN/cpu_capacity
// (Linux) or PROCESSOR_RELATIONSHIP::EfficiencyClass (Windows). On homogeneous
// hosts (Z1 Extreme, Ryzen 7 5800X, etc.) class info is uniform or absent and
// the bias is a no-op — the masks produced are bit-for-bit identical to the
// pre-Y-3 implementation.
//
...
//
// Y-3 (PERF/GR2FORK): on hybrid hosts the pick is biased toward the highest
// P-class core not already claimed by GpuComm; if no P-core remains the
// caller falls back to the highest unclaimed core regardless of class. On
// homogeneous hosts (the outcomes above) the result is unchanged.
//
