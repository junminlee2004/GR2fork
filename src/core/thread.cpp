...
#if defined(__linux__)
// PERF(GR2FORK Y-3): physical-core descriptor with performance class.
//
// `perf_class` mirrors the host's notion of core performance class:
//   * Linux: value of /sys/devices/system/cpu/cpuN/cpu_capacity for any
//     logical CPU in the SMT group. 0 if the file is absent (typical on
//     homogeneous AMD Zen 2/3/4 hosts including Z1 Extreme; also old
//     pre-5.13 kernels even on Intel hybrid). Intel Alder/Raptor/Meteor
//     Lake P-cores = 1024, E-cores ~= 447. ARM big.LITTLE also lands here.
//   * Windows: EfficiencyClass + 1 from PROCESSOR_RELATIONSHIP. 1 if all
//     cores share the same class (homogeneous); higher = more performant.
// 0 means "unknown" — the caller treats unknown as if all cores were the
// same class (no P/E differentiation).
struct PhysCore {
    u64 mask;
    u32 perf_class;
};

// Read /sys/devices/system/cpu/cpuN/cpu_capacity. Returns 0 if absent or
// unreadable. Cheap; called only during DecideReservedCores topology probe.
u32 ReadCpuCapacityLinux(unsigned cpu) {
    char path[160];
...
// Returns true iff the cores have >= 2 distinct nonzero perf_class values.
// Cores with perf_class == 0 (unknown) are treated as "ambiguous" — if ANY
// core's class is unknown we conservatively report not-hybrid. In practice
// the per-CPU cpu_capacity file is either present for every CPU or for
// none (on Linux); on Windows EfficiencyClass is always defined post-1809.
bool IsHybridTopology(const std::vector<PhysCore>& cores) {
    if (cores.empty()) return false;
    u32 first = 0;
    bool seen_value = false;
    for (const auto& c : cores) {
        if (c.perf_class == 0) {
            return false; // partial / unknown — defensively not-hybrid
        }
        if (!seen_value) {
            first = c.perf_class;
            seen_value = true;
        } else if (c.perf_class != first) {
            return true;
        }
    }
    return false;
...
// Maximum perf_class value across cores. 0 if every core is unknown.
u32 MaxPerfClass(const std::vector<PhysCore>& cores) {
    u32 m = 0;
    for (const auto& c : cores) {
        if (c.perf_class > m) m = c.perf_class;
    }
    return m;
}
#endif
...
#if defined(__linux__)
// Enumerate physical cores via /sys topology. Returns one entry per physical
...
// push the same physical core twice. Y-3: also reads cpu_capacity to tag
// each core with a perf_class for hybrid-aware picking.
std::vector<PhysCore> EnumeratePhysicalCoresLinux(unsigned hw) {
    std::vector<PhysCore> cores;
    ...
    for (unsigned cpu = 0; cpu < hw; ++cpu) {
        ...
        // Y-3: tag with cpu_capacity. All threads in an SMT group share a
        // physical core and therefore the same capacity, so reading from
        // `cpu` (the lowest-indexed thread in this group) is sufficient.
        const u32 cap = ReadCpuCapacityLinux(cpu);
        cores.push_back(PhysCore{sibs, cap});
...
// numbered physical core, matching the Linux ordering. Y-3: also extract
// EfficiencyClass for hybrid-aware picking.
std::vector<PhysCore> EnumeratePhysicalCoresWindows() {
    std::vector<PhysCore> cores;
    DWORD len = 0;
    ...
    while (reinterpret_cast<std::byte*>(p) < buf.data() + len) {
        if (p->Relationship == RelationProcessorCore && p->Processor.GroupCount > 0) {
            ...
            // Y-3: EfficiencyClass is a UCHAR: 0 = least performant on this
            // host; higher = more performant. On homogeneous CPUs every core
            // has EfficiencyClass == 0. We store as EfficiencyClass + 1 so
            // the value 0 is reserved for "unknown" (we always know on
            // Windows post-1809, but the +1 keeps the semantics uniform
            // with the Linux path where 0 means "/sys file absent").
            const u32 cls = static_cast<u32>(p->Processor.EfficiencyClass) + 1u;
            if (m != 0) cores.push_back(PhysCore{m, cls});
        }
        p = reinterpret_cast<SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX*>(
            reinterpret_cast<std::byte*>(p) + p->Size);
    }
    std::sort(cores.begin(), cores.end(), [](const PhysCore& a, const PhysCore& b) {
...
ReservedCoreDecision DecideReservedCores() {
    ...
    // Y-3 (PERF/GR2FORK): on hybrid hosts (Intel Alder/Raptor/Meteor Lake,
    // AMD Strix Point Zen5+Zen5c, ARM big.LITTLE) the legacy "highest
    // physical core" walk can land on an E-core, which is exactly the
    // wrong choice for GpuComm (documented single-threaded bottleneck)
    // and for the BundleAssembler (deadline-sensitive recorder draining
    // into vk command buffers). Detect P/E classes here and bias picking
    // toward P-class throughout. On homogeneous hosts (Z1 Extreme, Ryzen
    // 7 5800X, etc.) class info is uniform or absent and the bias is a
    // no-op: the helper falls through to the legacy "highest core" walk
    // and produces bit-for-bit identical masks to the pre-Y-3 code.
    const bool hybrid = IsHybridTopology(cores);
    const u32 max_class = MaxPerfClass(cores);

    {
        // One-time topology summary in the log so hybrid-host users can
        // verify the P/E classification was picked up correctly. Gated by
        // call_once because DecideReservedCores is invoked multiple times
        // (no internal cache) and we don't want log spam.
        static std::once_flag logged;
        std::call_once(logged, [&]() {
            if (cores.empty()) {
                LOG_INFO(Common,
                         "[GR2FORK Y-3] topology probe: hw={} no phys core "
                         "info available", hw);
                return;
            }
            unsigned p_cores = 0, e_cores = 0;
            for (const auto& c : cores) {
                if (hybrid && c.perf_class == max_class) ++p_cores;
                else if (hybrid) ++e_cores;
            }
            if (hybrid) {
                LOG_INFO(Common,
                         "[GR2FORK Y-3] hybrid host detected: {} P-cores, "
                         "{} E-cores ({} phys total, max_class={})",
                         p_cores, e_cores, cores.size(), max_class);
            } else {
                LOG_INFO(Common,
                         "[GR2FORK Y-3] homogeneous host: {} phys cores "
                         "(class info: {})",
                         cores.size(),
                         max_class != 0 ? "uniform" : "unavailable");
            }
        });
    }

    // Y-3: helper to walk cores from highest-numbered to lowest, preferring
    // P-class on hybrid hosts. Returns an iterator into `cores`, or
    // cores.rend() if no candidate satisfies the exclude_mask filter.
    //
    // On homogeneous hosts the hybrid-branch is skipped and the function
    // reduces to "first non-overlapping core walking rbegin->rend", which
    // matches the pre-Y-3 cores.back()/loop behavior exactly.
    auto pick_highest = [&](u64 exclude_mask) {
        if (hybrid) {
            for (auto it = cores.rbegin(); it != cores.rend(); ++it) {
                if (it->mask == 0) continue;
                if ((it->mask & exclude_mask) != 0) continue;
                if (it->perf_class == max_class) return it;
            }
            // No P-core left; fall through to legacy "any highest core" walk.
            // Acceptable on hosts with very few P-cores (e.g. 2P+8E i3
            // configurations where both P-cores are already in the guest's
            // claimed range) — we still want a pin even if it's on an E.
        }
        for (auto it = cores.rbegin(); it != cores.rend(); ++it) {
            if (it->mask == 0) continue;
            if ((it->mask & exclude_mask) != 0) continue;
    ...
    // Y-3: look up CPU 7's physical-core perf_class so we can reject
    // Phase 1 on hybrid hosts where CPU 7 lands on an E-core.
    u32 cpu7_class = 0;
    for (const auto& c : cores) {
        if (c.mask == cpu7_phys) {
            cpu7_class = c.perf_class;
            break;
        }
    }

    bool phase1_ok = (cpu7_phys != 0 && (cpu7_phys & (1ULL << 7)) != 0
                      && (cpu7_phys & GUEST_MASK) == 0);
    if (phase1_ok && hybrid && cpu7_class != 0 && cpu7_class < max_class) {
        // Y-3: CPU 7's phys core is free of guest contention but it's an
        // E-class core. On hybrid hosts we want GpuComm on a P-core; force
        // the Phase 2 path which prefers P-cores. The exclusion walk in
        // Phase 2 will steal the chosen P-core from the guest mask via
        // Pthread::SetAffinity bit-stripping.
        //
        // This is the exotic-layout defense — Intel hybrid Linux typically
        // lays out P-cores first with HT (CPUs 0-15 on 8P+8E), so CPU 7's
        // sibling is CPU 6 (in guest range) and phase1_ok is already false
        // by the GUEST_MASK overlap check. But an E-cores-first BIOS/Linux
        // layout would expose CPU 7 as a free E-core, and this branch
        // catches that.
        static std::once_flag once;
        std::call_once(once, [&]() {
            LOG_INFO(Common,
                     "[GR2FORK Y-3] CPU 7's phys core is E-class "
                     "(perf_class={}, max={}); skipping Phase 1, stealing "
                     "a P-core via Phase 2",
                     cpu7_class, max_class);
        });
        phase1_ok = false;
    }
    ...
    if (phase1_ok) {
        ...
        // ----- Phase 2: enumerate physical cores and steal one -----
        ...
        //   * Y-3: any hybrid host where Phase 1 was rejected because
        //          CPU 7 was on an E-class core.
        //
        // Y-3: pick the highest P-class core not overlapping the guest's
        // current claim (`pick_highest(0)` searches the full host range,
        // since the guest steal happens AFTER this pick via the
        // `steal_from_guest` field). On homogeneous hosts pick_highest
        // returns the same core as the legacy cores.back() walk.
        const auto it = pick_highest(0);
        if (it == cores.rend() || it->mask == 0) {
            return result;
        }
        result.mask = it->mask;
        result.is_dedicated_physical_core = true;
        result.steal_from_guest = it->mask;
    }

    // ----- Y-2 / Y-3: assembler reservation, P-class preferred -----
    ...
    //
    // Y-3 outcomes (hybrid hosts, P-class preferred for both):
    //
    //   * 8P+8E Intel 13700K Linux (P-cores HT on CPUs 0..15, E on 16..23):
    //                                 GpuComm = highest P phys core {14,15},
    //                                 assembler = next-highest P {12,13},
    //                                 rest = P cores 0..5 (12 logical) +
    //                                        E cores 16..23 (8 logical).
    //   * 6P+8E Intel 12700K:         GpuComm = highest P {10,11},
    //                                 assembler = next-highest P {8,9},
    //                                 rest = P cores 0..3 + all 8 E cores.
    //   * 4P+8E Intel 12400:          GpuComm = highest P {6,7},
    //                                 assembler = next-highest P {4,5},
    //                                 rest = P cores 0..1 + 8 E cores.
    //                                 (Note: Phase 1 may take {6,7} directly
    //                                 if it's a P-class and guest-free; on
    //                                 Linux's standard HT-pairs layout it is.)
    //
    ...
    if (cores.size() >= 4 && result.mask != 0) {
        // Y-3: walk uses the same P-class-preferring helper as GpuComm,
        // excluding GpuComm's mask. On homogeneous hosts this picks the
        // second-highest physical core — identical to the legacy walk.
        const auto it = pick_highest(result.mask);
        if (it != cores.rend() && it->mask != 0) {
            result.assembler_mask = it->mask;
            ...
            result.assembler_steal_from_guest = (it->mask & GUEST_MASK);
        }
    }
