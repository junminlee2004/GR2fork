...
static bool SymInitSafe() {
    ...
    // SymInitialize with fInvadeProcess=TRUE auto-enumerates loaded modules.
    if (!SymInitialize(GetCurrentProcess(),
                       search_path[0] ? search_path : nullptr, TRUE)) {
        // Another component (likely HangWatchdog) may already have
        // inited — this is expected and fine, symbol state is shared.
        return true;
    }
    return true;
}
