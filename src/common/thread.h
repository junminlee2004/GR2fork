...
// PERF(ISSFORK): pin the calling thread to a set of host CPUs.
//
...
// and FreeBSD's cpuset_setaffinity is out of scope for the initial ISSFORK
// affinity work.
bool SetCurrentThreadAffinityMask(u64 mask);

// PERF(ISSFORK): mask of host CPUs to pin a hot emulator thread onto.
//
...
// PERF(ISSFORK): bits to STRIP from guest-thread affinity masks when
...
// PERF(ISSFORK): bits to OR into every guest-thread affinity mask, in
...
// PERF(ISSFORK): grant the reserved cores EXCLUSIVELY to the calling thread,
...
// PERF(ISSFORK): start (once, idempotently) a detached background thread
