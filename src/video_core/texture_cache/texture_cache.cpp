...
bool TextureCache::IsUmaWritebackCandidate(const Image& image) const {
    // Small low-bit-depth color surfaces - the CPU-feedback shape
    // (visibility maps, classification grids, simulation results). Anything
    // wider is an ordinary render target the CPU never consumes; writing
    // those back at every fence drowns the frame in copies.
    ...
    if (image.info.guest_address == 0 || image.info.props.is_depth || image.info.num_samples != 1 ||
        image.info.num_bits > WritebackMaxBits || image.info.guest_size > WritebackSizeLimit) {
        return false;
    }
    return true;
}
...
void TextureCache::WritebackImageUma(ImageId image_id) {
    ...
    const auto resolved =
        buffer_cache.ResolveGuest(image.info.guest_address, image.info.guest_size);
    if (!resolved) {
    ...
    if (auto barrier = buffer_cache.RequestUnifiedBarrier(
            window, offset, image.info.guest_size, vk::AccessFlagBits2::eTransferWrite,
    ...
    tile_manager.TileImage(image, copies, buffer_cache.UnifiedWindow(window).Handle(), offset,
                           copy_size);
    buffer_cache.MarkUnifiedSelfSynced(window, offset, image.info.guest_size);
...
void TextureCache::RefreshImage(Image& image) {
    ...
    const auto barrier =
        in_buffer->is_unified
            ? buffer_cache.RequestUnifiedBarrier(
                  buffer_cache.WindowIndex(in_buffer), in_offset, image.info.guest_size,
                  vk::AccessFlagBits2::eTransferRead, vk::PipelineStageFlagBits2::eTransfer, false)
            : in_buffer->GetBarrier(vk::AccessFlagBits2::eTransferRead,
