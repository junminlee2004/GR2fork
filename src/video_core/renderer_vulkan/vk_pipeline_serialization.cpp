...
bool PipelineCache::LoadComputePipeline(Serialization::Archive& ar) {
    ...
    it.value() = std::make_unique<ComputePipeline>(instance, scheduler, desc_heap, desc_set_cache,
                                                   profile, *pipeline_cache, compute_key, *infos[0],
                                                   modules[0], sdata, true);
...
bool PipelineCache::LoadGraphicsPipeline(Serialization::Archive& ar) {
    ...
    it.value() = std::make_unique<GraphicsPipeline>(
        ...
        infos, runtime_infos, std::move(fetch_data), modules, sdata, true);
